...
function testSubmitFormStopsBeforeCatchWhenValidationFails() {
  assert.match(
    ...
    'expected submitForm to normalize form validation failures into a false result'
  )

  assert.match(
    source,
    /if\s*\(!valid\)\s*return/,
    'expected submitForm to return early when the form is invalid'
  )
}

async function main() {
  testSubmitFormStopsBeforeCatchWhenValidationFails()
  console.log('config accountauth validation guard tests passed')
}
