...
export default {
    ...
    // 文件上传
    getTerminalAuditSummary() {
        return request({
            url: 'terminalAudit/summary',
            method: 'get'
        })
    },
    queryTerminalAuditSessionList(params) {
        return request({
            url: 'terminalAudit/list',
            method: 'get',
            params: params
        })
    },
    getTerminalAuditSessionDetail(sessionId) {
        return request({
            url: `terminalAudit/session/${encodeURIComponent(sessionId)}`,
            method: 'get'
        })
    },
    getTerminalAuditSessionPlayback(sessionId, params) {
        return request({
            url: `terminalAudit/session/${encodeURIComponent(sessionId)}/playback`,
            method: 'get',
            params
        })
    },
    getTerminalAuditRecordingDownloadUrl(sessionId, token) {
        const encodedSessionId = encodeURIComponent(sessionId)
        const tokenValue = token ? `?token=${encodeURIComponent(token)}` : ''
        return `/api/v1/terminalAudit/session/${encodedSessionId}/download${tokenValue}`
    },
    getUploadUrl() {
