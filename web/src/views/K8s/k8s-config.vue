...
import { buildConfigMapCreatePayload, buildSecretCreatePayload } from '@/utils/k8sManifest.mjs'
...
const handleConfigMapYamlSave = async (data) => {
  try {
    ...
    let response
    if (isCreating) {
      const payload = buildConfigMapCreatePayload(data.yamlContent)
      response = await k8sApi.createConfigMap(selectedClusterId.value, queryParams.namespace, payload)
    } else {
      // 更新现有ConfigMap
...
const handleSecretYamlSave = async (data) => {
  try {
    ...
    let response
    if (isCreating) {
      const payload = buildSecretCreatePayload(data.yamlContent)
      response = await k8sApi.createSecret(selectedClusterId.value, queryParams.namespace, payload)
    } else {
      // 更新现有Secret
