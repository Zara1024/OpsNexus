...
watch(() => props.workload, (newWorkload) => {
  if (newWorkload && Object.keys(newWorkload).length > 0) {
    ...
    // 初始化环境变量
    configForm.envVars = (newWorkload.envVars || []).map(env => ({
      name: env.name || '',
      value: env.value ?? ''
    }))
  }
...
const handleSubmit = async () => {
  ...
  try {
    emit('submit', {
      cpuRequest: configForm.cpuRequest?.trim() || '',
      cpuLimit: configForm.cpuLimit?.trim() || '',
      memoryRequest: configForm.memoryRequest?.trim() || '',
      memoryLimit: configForm.memoryLimit?.trim() || '',
      envVars: configForm.envVars
        .map(env => ({
          name: env.name?.trim() || '',
          value: env.value ?? ''
        }))
