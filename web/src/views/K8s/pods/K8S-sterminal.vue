...
// 从路由参数获取Pod信息
const clusterId = computed(() => props.clusterId || route.params.clusterId)
const namespace = computed(() => props.namespace || route.params.namespace)
const podName = computed(() => props.podName || route.params.podName)
const containerFromQuery = computed(() => props.container || route.query.container)
...
watch(
  () => [clusterId.value, namespace.value, podName.value, containerFromQuery.value],
  async (newValues, oldValues) => {
    if (!oldValues || newValues.join('|') === oldValues.join('|')) {
      return
    }
...
.terminal-container.embedded :deep(.el-card) {
  height: 100%;
  border: none;
  box-shadow: none;
}
...
.terminal-container.embedded .terminal-content {
  height: 460px;
}
