...
*/
const navigateToPodDetail = async () => {
  try {
    ...
    if (!Array.isArray(pods) || pods.length === 0) {
      ElMessage.warning('该工作负载下暂无可用 Pod')
      return
    }

    router.push({
      path: `/k8s/pod/${props.clusterId}/${props.namespace}/${pods[0].name}`
    })
  } catch (error) {
    console.error('跳转到 Pod 详情失败:', error)
    ElMessage.error('跳转到 Pod 详情失败')
  }
}
