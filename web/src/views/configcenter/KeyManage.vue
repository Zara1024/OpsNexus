<template>
  <div class="key-management">
    <el-card shadow="hover" class="key-card">
          ...
          <el-table-column label="操作" width="200" fixed="right">
            <template #default="{ row }">
              <div class="operation-buttons">
                ...
                <el-tooltip content="手动同步日志" placement="top">
                  <el-button size="small" type="info" icon="Document" circle @click="showManualSyncLog(row)" />
                </el-tooltip>
      ...
      <!--新增/编辑对话框-->
      <el-dialog :title="dialogTitle" v-model="dialogVisible" width="35%" :modal="false" destroy-on-close>
      ...
      <el-dialog title="手动同步日志" v-model="manualSyncLogDialogVisible" width="50%" :modal="false">
        <div v-loading="manualSyncLogLoading" class="sync-log-container">
          <div v-if="manualSyncLogs.length" class="log-info">
            <div class="log-content">
              <div class="log-title">最近手动同步记录:</div>
              <div v-for="item in manualSyncLogs" :key="item.id" class="log-header-item">
                <span class="log-label">{{ item.createTime || '-' }}</span>
                <span class="log-value">{{ item.description || item.url }}</span>
              </div>
            </div>
          </div>
          <el-empty v-else description="暂无手动同步日志" />
        </div>
        <template #footer>
          <el-button @click="manualSyncLogDialogVisible = false">关闭</el-button>
        </template>
      </el-dialog>
...
<script>
import { Setting, Key, Lock, Timer } from '@element-plus/icons-vue'
import API from '@/api/config'
import systemAPI from '@/api/system'
import { extractManualSyncLogs } from '@/utils/keyManualSyncLog.mjs'
...
export default {
  ...
  data() {
    return {
      ...
      currentSyncLog: null,
      manualSyncLogDialogVisible: false,
      manualSyncLogLoading: false,
      manualSyncLogs: [],
      keyTypeOptions: [
        { value: 1, label: '阿里云' },
        { value: 2, label: '腾讯云' },
...
async submitScheduleForm() {
      try {
        const valid = await this.$refs.scheduleFormRef.validate().catch(() => false)
        if (!valid) return
    ...
    async showManualSyncLog() {
      this.manualSyncLogDialogVisible = true
      this.manualSyncLogLoading = true
      this.manualSyncLogs = []
      try {
        const { data: res } = await systemAPI.querySysOperationLogList({
          pageNum: 1,
          pageSize: 20
        })
        if (res.code === 200) {
          const rows = res.data?.list || []
          this.manualSyncLogs = extractManualSyncLogs(rows)
        } else {
          this.$message.error(res.message || '获取手动同步日志失败')
        }
      } catch (error) {
        console.error('获取手动同步日志失败:', error)
        this.$message.error('获取手动同步日志失败')
      } finally {
        this.manualSyncLogLoading = false
      }
    },

    async handleDeleteSchedule(row) {
      try {
        await this.$confirm(`纭畾鍒犻櫎瀹氭椂鍚屾閰嶇疆"${row.name}"?`, '鎻愮ず', {
    ...
    async handleSyncHosts(row) {
      try {
        await this.$confirm(`纭畾鍚屾${this.getKeyTypeTag(row.keyType).label}涓绘満?`, '鎻愮ず', {
          confirmButtonText: '纭畾',
          cancelButtonText: '鍙栨秷',
          type: 'info'
        })
    ...
    async handleDelete(row) {
      try {
        await this.$confirm(`纭畾鍒犻櫎璇ュ瘑閽ラ厤缃?`, '鎻愮ず', {
          confirmButtonText: '纭畾',
          cancelButtonText: '鍙栨秷',
          type: 'warning'
        })
