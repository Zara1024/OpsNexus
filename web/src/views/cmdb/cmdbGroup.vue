  ...
  <div class="cmdb-group-management">
    <el-card shadow="hover" class="group-card">
      ...
      <div class="search-section">
        <el-form :inline="true" :model="queryParams" ref="queryForm" class="search-form" @submit.prevent>
...
export default {
  ...
  methods: {
    ...
    normalizeParentId(parentId) {
      return parentId === undefined || parentId === null || parentId === '' ? 0 : parentId
    },
    // 获取分组列表
    async getList() {
      this.loading = true
    ...
    async addGroup() {
      try {
        ...
        const payload = {
          ...this.addGroupForm,
          parentId: this.normalizeParentId(this.addGroupForm.parentId)
        }
        const { data: res } = await API.createCmdbGroup(payload)
        if (res.code !== 200) {
      ...
      const performUpdate = async () => {
        try {
          ...
          const { data: res } = await API.updateCmdbGroup({
            ...this.groupInfo,
            parentId: this.normalizeParentId(this.groupInfo.parentId),
...
.modern-table :deep(.el-table__row td .cell) {
...
.level-2-group {
  ...
  padding-left: 8px;
  background: linear-gradient(90deg, rgba(148, 163, 184, 0.08) 0%, rgba(148, 163, 184, 0) 100%);
}
