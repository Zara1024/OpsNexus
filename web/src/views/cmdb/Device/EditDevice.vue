...
function parseProtocolGroup(value) {
  const protocols = String(value || '')
    .split(',')
    .map((item) => item.trim().toLowerCase())
    .filter(Boolean)

  return protocols.length ? protocols : ['ssh']
}
...
export default {
  ...
  methods: {
    initForm() {
      const base = createDeviceAssetFormModel()
      this.form = {
        ...base,
        ...this.deviceInfo,
        id: this.deviceInfo?.id ?? base.id,
        groupId: this.deviceInfo?.groupId ?? base.groupId,
        accountId: this.deviceInfo?.accountId ?? base.accountId,
        isActive: this.deviceInfo?.isActive ?? base.isActive,
        sshPort: Number(this.deviceInfo?.sshPort || base.sshPort),
        telnetPort: Number(this.deviceInfo?.telnetPort || base.telnetPort)
      }
      this.protocolGroupSelection = parseProtocolGroup(this.deviceInfo?.protocolGroup)
      this.$nextTick(() => {
    ...
    async submitForm() {
      try {
        ...
        console.error('设备编辑表单校验失败:', error)
      }
    }
  }
}
