<template>
  ...
  >
    ...
    <div v-if="!monitorPanelState.ready" class="port-empty-state">
      {{ monitorPanelState.reason }}
    </div>
...
export default {
  ...
  computed: {
    ...
    monitorPanelState() {
      return getHostMonitorPanelState(this.portData, ['ports'], '暂无端口监控数据')
    }
  },
  watch: {
    visible(newVal) {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          this.portData = {
            ...
            unavailableReason: res.message || '端口监控暂不可用'
          }
