<template>
  ...
  >
    ...
    <el-alert
      v-if="!monitorDataAvailable"
      :title="monitorUnavailableReason || '监控链路未接入'"
      type="warning"
      :closable="false"
      class="monitor-alert"
    />
    ...
    <el-row :gutter="20" class="monitor-row">
      <!-- 普通指标卡片 -->
      <el-col :span="8" v-for="metric in nonNetworkIOMetrics" :key="metric.key">
        <el-card class="metric-card">
          <div class="metric-header">
            <span class="metric-title">{{ metric.title }}</span>
            <el-tag size="mini" :type="getMetricStatus(metric.value)">
              {{ formatMetricTagValue(metric) }}
            </el-tag>
          </div>
          ...
          <div class="metric-header">
            ...
            <div class="disk-io-tags">
              <el-tag size="mini" type="success" style="margin-right: 5px;">
                读: {{ formatMetricTagValue(diskReadMetric) }}
              </el-tag>
              <el-tag size="mini" type="warning">
                写: {{ formatMetricTagValue(diskWriteMetric) }}
              </el-tag>
            </div>
          </div>
          ...
          <div class="metric-header">
            ...
            <div class="network-io-tags">
              <el-tag size="mini" type="info" style="margin-right: 5px;">
                接收: {{ formatMetricTagValue(networkReceiveMetric) }}
              </el-tag>
              <el-tag size="mini" type="primary">
                发送: {{ formatMetricTagValue(networkSendMetric) }}
              </el-tag>
            </div>
          </div>
...
import {
  ...
  hasHostMonitorData
} from '@/utils/hostMonitorState.mjs'
import {
  centerSinglePointSeries,
  computeChartAxisMax
} from '@/utils/monitorChartPresentation.mjs'
...
export default {
  ...
  data() {
    return {
      ...
      monitorDataAvailable: false,
      monitorCollectionStatus: 'unavailable',
      monitorUnavailableReason: '监控链路未接入'
    }
  },
  computed: {
    dialogVisible: {
    ...
    // 磁盘读取指标
    diskReadMetric() {
      return this.metrics.find(metric => metric.key === 'diskReadKB') || { value: null, unit: 'KB/s' }
    },
    // 磁盘写入指标
    diskWriteMetric() {
      return this.metrics.find(metric => metric.key === 'diskWriteKB') || { value: null, unit: 'KB/s' }
    },
    // 网络接收指标
    networkReceiveMetric() {
      return this.metrics.find(metric => metric.key === 'networkReceive') || { value: null, unit: 'KB/s' }
    },
    // 网络发送指标
    networkSendMetric() {
      return this.metrics.find(metric => metric.key === 'networkSend') || { value: null, unit: 'KB/s' }
    }
  },
  methods: {
    ...
    formatMetricTagValue(metric) {
      ...
      return `${formatHostMonitorMetricValue(metric?.value)}${suffix}`.trim()
    },
    resetMetricValues(fallback = null) {
      this.metrics.forEach(metric => {
        metric.value = fallback
      })
    },
    applyUnavailableMetrics(metricsData) {
      const reason = getHostMonitorUnavailableReason(metricsData)
      this.monitorDataAvailable = false
      this.monitorCollectionStatus = metricsData?.collectionStatus || 'unavailable'
      this.monitorUnavailableReason = reason
      this.resetMetricValues(null)
      this.metrics.forEach(metric => {
        this.createEmptyChart(metric.key, metric, reason)
      })
      this.createEmptyChart('diskIO', { key: 'diskIO', title: '磁盘I/O速率' }, reason)
      this.createEmptyChart('networkIO', { key: 'networkIO', title: '网络I/O速率' }, reason)
    },
    async fetchMetrics() {
      try {
        ...
        if (response.data.code === 200) {
          ...
          this.monitorDataAvailable = hasHostMonitorData(payload)
          this.monitorCollectionStatus = payload.collectionStatus || 'unavailable'
          this.monitorUnavailableReason = getHostMonitorUnavailableReason(payload)
          await this.updateMetrics(payload)
        }
      } catch (error) {
        console.error('获取监控指标失败:', error)
        this.applyUnavailableMetrics({
          collectionStatus: 'unavailable',
          unavailableReason: error?.message || '监控链路未接入'
        })
      }
    },
    async updateMetrics(metricsData) {
      ...
      if (!hasHostMonitorData(metricsData)) {
        this.applyUnavailableMetrics(metricsData)
        return
      }
    ...
    renderSingleChart(chartDom, metric, xAxisData, seriesData) {
      try {
        ...
        const timeline = centerSinglePointSeries(xAxisData, seriesData)
        const isSnapshotOnly = timeline.snapshotOnly
        ...
        const option = {
          ...
          series: [{
            ...
            smooth: true,
            symbol: 'circle',
            showSymbol: isSnapshotOnly,
            symbolSize: isSnapshotOnly ? 14 : 6,
            lineStyle: {
              color: '#00d8ff',
              width: isSnapshotOnly ? 0 : 2
            },
            itemStyle: {
              color: '#00d8ff',
              shadowBlur: isSnapshotOnly ? 14 : 0,
              shadowColor: isSnapshotOnly ? 'rgba(0, 216, 255, 0.45)' : 'transparent'
            },
            areaStyle: {
              opacity: isSnapshotOnly ? 0 : 1,
              color: {
                type: 'linear',
                x: 0,
                y: 0,
          ...
          graphic: isSnapshotOnly ? [{
            type: 'text',
            right: 18,
            top: 36,
            style: {
              text: `最新采样 ${timeline.latestLabel}`,
              fill: '#94a3b8',
              fontSize: 11
            }
          }] : []
    ...
    renderDiskIOChart(metricsData) {
      try {
        ...
        const timeline = centerSinglePointSeries(xAxisData, readSeriesData.length ? readSeriesData : writeSeriesData)
        const readValues = timeline.snapshotOnly ? [null, readSeriesData[0] ?? null, null] : readSeriesData
        const writeValues = timeline.snapshotOnly ? [null, writeSeriesData[0] ?? null, null] : writeSeriesData
        const axisMax = computeChartAxisMax([readSeriesData, writeSeriesData])
        ...
        const option = {
          ...
          series: [
            {
              ...
              areaStyle: {
                opacity: timeline.snapshotOnly ? 0 : 1,
                color: {
                  type: 'linear',
                  x: 0,
                  y: 0,
          ...
          graphic: timeline.snapshotOnly ? [{
            type: 'text',
            right: 18,
            top: 52,
            style: {
              text: `???? ${timeline.latestLabel}`,
              fill: '#94a3b8',
              fontSize: 11
            }
          }] : []
    ...
    renderNetworkIOChart(metricsData) {
      try {
        ...
        const timeline = centerSinglePointSeries(
          xAxisData,
          receiveSeriesData.length ? receiveSeriesData : sendSeriesData
        )
        const receiveValues = timeline.snapshotOnly
          ? [null, receiveSeriesData[0] ?? null, null]
          : receiveSeriesData
        const sendValues = timeline.snapshotOnly
          ? [null, sendSeriesData[0] ?? null, null]
          : sendSeriesData
        const axisMax = computeChartAxisMax([receiveSeriesData, sendSeriesData])
        ...
        const option = {
          ...
          tooltip: {
            ...
            axisPointer: {
              ...
              lineStyle: {
                color: '#00d8ff',
                width: timeline.snapshotOnly ? 0 : 2,
                opacity: 0.8
              }
            },
            formatter: function(params) {
          ...
          series: [
            {
              ...
              smooth: true,
              symbol: 'circle',
              showSymbol: timeline.snapshotOnly,
              symbolSize: timeline.snapshotOnly ? 12 : 6,
              lineStyle: {
                color: '#17a2b8',
                width: timeline.snapshotOnly ? 0 : 2
              },
    ...
    createEmptyChart(chartKey, metric, message = '暂无数据') {
      try {
        // 获取图表容器
