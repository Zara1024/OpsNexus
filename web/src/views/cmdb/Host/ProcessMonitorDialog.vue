    ...
    <el-row :gutter="20" class="process-charts">
      <el-col :span="12">
        <el-card class="chart-card">
          ...
          <div v-if="hasChartData(processData.topCPU)" class="chart-container" ref="cpuChart"></div>
          <div v-else class="chart-empty">{{ monitorPanelState.reason }}</div>
        </el-card>
      </el-col>
      <el-col :span="12">
        <el-card class="chart-card">
          ...
          <div v-if="hasChartData(processData.topMemory)" class="chart-container" ref="memoryChart"></div>
          <div v-else class="chart-empty">{{ monitorPanelState.reason }}</div>
        </el-card>
      </el-col>
    </el-row>
...
import { getHostMonitorPanelState } from '@/utils/hostMonitorState.mjs'
...
export default {
  ...
  computed: {
    ...
    monitorPanelState() {
      return getHostMonitorPanelState(this.processData, ['topCPU', 'topMemory'], '暂无进程监控数据')
    }
  },
  watch: {
    visible(newVal) {
  ...
  methods: {
    ...
    renderCpuChart() {
      if (!this.hasChartData(this.processData.topCPU) || !this.$refs.cpuChart) return
    ...
    renderMemoryChart() {
      if (!this.hasChartData(this.processData.topMemory) || !this.$refs.memoryChart) return
