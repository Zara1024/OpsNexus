    ...
    <el-card shadow="hover" class="dblog-card">
      ...
      <div class="table-section">
        <el-table v-loading="Loading" :data="sqlLogList" stripe style="width: 100%"
          ...
          <el-table-column label="SQL内容" prop="sqlContent" min-width="180">
            <template #default="scope">
              <el-tooltip :content="scope.row.sqlContent || '-'" placement="top" :show-after="300">
                <span>{{ formatCompactLabel(scope.row.sqlContent, 24, 8) }}</span>
              </el-tooltip>
            </template>
          </el-table-column>
          ...
          <el-table-column label="查询时间" prop="queryTime" min-width="160" />
...
export default {
  ...
  methods: {
    formatCompactLabel(value, prefixLength = 24, suffixLength = 8) {
    ...
    async batchHandleDelete() {
      ...
      const { data: res } = await this.$api.BatchDeleteCmdbSqlLog(sqlLogIds)
      if (res.code !== 200) {
        this.$message.error(res.message)
      } else {
        this.$message.success('删除成功')
...
.dblog-table :deep(.cell.el-tooltip) {
  white-space: normal;
