              ...
              <div class="step-actions">
                ...
                <el-tooltip content="重新执行" placement="top">
                  <el-button
                    ...
                    :loading="retryingStepName === step.name"
...
const retryStep = (step) => {
  ElMessageBox.confirm(
    `确定要重新执行步骤“${step.name}”吗？当前会重新拉起整个 Ansible 任务流程。`,
    '重新执行确认',
    {
    ...
    retryingStepName.value = step.name
    try {
      const response = await StartAnsibleTask(props.taskId)
      if (response?.data?.code === 200) {
        ElMessage.success(`已重新执行任务，来源步骤：${step.name}`)
        await fetchTaskData()
      } else {
        ElMessage.error(response?.data?.message || '重新执行失败')
      }
    } catch (error) {
      console.error('重新执行任务失败:', error)
      ElMessage.error('重新执行任务失败')
    } finally {
      retryingStepName.value = ''
    }
  }).catch(() => {})
}
