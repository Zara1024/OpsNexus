...
const formatLogContent = (content) => {
  ...
  // ANSI颜色代码转换为HTML样式
  return content
    .replace(new RegExp(`${ansiEscape}\\[0;32m`, 'g'), '<span style="color: #67c23a;">')  // 绿色
    .replace(new RegExp(`${ansiEscape}\\[0;31m`, 'g'), '<span style="color: #f56c6c;">')  // 红色
    .replace(new RegExp(`${ansiEscape}\\[0;33m`, 'g'), '<span style="color: #e6a23c;">')  // 黄色
    .replace(new RegExp(`${ansiEscape}\\[0;34m`, 'g'), '<span style="color: #409eff;">')  // 蓝色
    .replace(new RegExp(`${ansiEscape}\\[0m`, 'g'), '</span>')                              // 结束颜色
    .replace(new RegExp(`${ansiEscape}\\[[0-9;]*m`, 'g'), '')                               // 清除其他ANSI代码
}
