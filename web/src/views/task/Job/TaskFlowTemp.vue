...
<path fill="#60A5FA" d="M512 1024C229.23 1024 0 794.77 0 512S229.23 0 512 0s512 229.23 512 512-229.23 512-512 512zm0-938.67C276.36 85.33 85.33 276.36 85.33 512S276.36 938.67 512 938.67 938.67 747.64 938.67 512 747.64 85.33 512 85.33z"/>
<path fill="#60A5FA" d="M341.33 426.67a42.67 42.67 0 100-85.34 42.67 42.67 0 000 85.34zM682.67 426.67a42.67 42.67 0 100-85.34 42.67 42.67 0 000 85.34z"/>
<path fill="#60A5FA" d="M512 768c-117.82 0-213.33-95.51-213.33-213.33h426.66C725.33 672.49 629.82 768 512 768z"/>
          </svg>
          {{ step.title }}
        </h3>
        ...
        <div class="card-buttons">
          ...
          <el-tooltip effect="dark" content="日志" placement="top">
            <button @click.stop="onLogClick(step)" class="btn-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M21 10H7M21 6H3M21 14H3M21 18H7" stroke="#38BDF8" stroke-width="2" stroke-linecap="round"/>
              </svg>
            </button>
          </el-tooltip>
...
.flow-card.status-completed {
  background: linear-gradient(180deg, rgba(7, 28, 22, 0.98), rgba(6, 20, 16, 0.95)) !important;
}

.flow-card.status-active {
  background: linear-gradient(180deg, rgba(10, 29, 54, 0.98), rgba(8, 20, 38, 0.95)) !important;
}

.flow-card.status-pending {
  background: linear-gradient(180deg, rgba(34, 24, 12, 0.98), rgba(20, 14, 8, 0.95)) !important;
}

.flow-card.status-error {
  background: linear-gradient(180deg, rgba(42, 15, 20, 0.98), rgba(24, 10, 12, 0.95)) !important;
}
...
.flow-card::before {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(circle at top left, rgba(59, 130, 246, 0.18), transparent 46%);
  z-index: -1;
}
