        ...
        <el-card shadow="hover" class="profile-card">
          ...
          <div class="profile-content">
            <div class="avatar-section">
              <el-avatar :src="displayAvatar" :size="130" :key="avatarKey" class="profile-avatar"></el-avatar>
            </div>
                ...
                <el-form :model="adminDetail" :rules="editFormRules" ref="editFormRefForm" label-width="100px" class="edit-form">
                  <el-form-item label="用户头像" prop="icon">
                    <el-upload :headers="headers" class="avatar-uploader" :action="uploadIconUrl"
                      ...
                      <img :src="displayAvatar" :key="avatarKey" class="avatar" title="点击更换头像">
                    </el-upload>
                  </el-form-item>
...
export default {
  ...
  computed: {
    resolvedAdminIcon() {
      return resolveAvatarUrl(this.adminDetail?.icon)
    },
    displayAvatar() {
      return this.icon || this.resolvedAdminIcon || require('@/assets/image/touxiang.jpg')
    }
  },
  methods: {
    // 成功调用图片信息
...
.personal-management :deep(.el-form-item.is-required:not(.is-no-asterisk) > .el-form-item__label-wrap > .el-form-item__label::before),
.personal-management :deep(.el-form-item.is-required:not(.is-no-asterisk) > .el-form-item__label::before) {
  color: #f87171;
}
...
.personal-management :deep(.el-input__wrapper),
.personal-management :deep(.el-textarea__inner) {
  background: rgba(15, 23, 42, 0.78);
  border: 1px solid rgba(148, 163, 184, 0.24);
  box-shadow: inset 0 0 0 1px rgba(15, 23, 42, 0.08);
}

.personal-management :deep(.el-input__wrapper:hover),
.personal-management :deep(.el-textarea__inner:hover) {
  border-color: rgba(191, 219, 254, 0.4);
}

.personal-management :deep(.is-focus.el-input__wrapper),
.personal-management :deep(.el-textarea__inner:focus) {
  border-color: rgba(96, 165, 250, 0.8);
  box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.16);
}

.personal-management :deep(.el-input__inner),
.personal-management :deep(.el-textarea__inner) {
  color: var(--text-primary);
...
.personal-management :deep(.el-input__inner::placeholder),
.personal-management :deep(.el-textarea__inner::placeholder) {
  color: var(--text-muted);
  -webkit-text-fill-color: var(--text-muted);
}

.personal-management :deep(.el-input.is-disabled .el-input__wrapper),
.personal-management :deep(.el-input.is-disabled .el-input__inner) {
  color: var(--text-primary);
  -webkit-text-fill-color: currentColor;
