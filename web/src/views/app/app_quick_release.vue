            ...
            <template #default="{ row }">
              <div class="table-cell-with-icon">
                ...
                <el-tooltip :content="row.title" placement="top" :show-after="300">
                  <span class="release-title-text">{{ formatCompactLabel(row.title, 8, 3) }}</span>
                </el-tooltip>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="applications" label="发布服务" min-width="240">
            <template #default="{ row }">
              <div class="services-cell">
                <template v-if="row.tasks && row.tasks.length > 0">
                  <div
                    ...
                    class="service-item">
                    <el-tooltip :content="task.app_name" placement="top" :show-after="300">
                      <span class="service-name">{{ formatCompactLabel(task.app_name, 8, 3) }}</span>
                    </el-tooltip>
                ...
                <template v-else-if="row.applications && row.applications.length > 0">
                  <div
                    ...
                    class="service-item">
                    <el-tooltip :content="app.app_name || `服务${app.app_id}`" placement="top" :show-after="300">
                      <span class="service-name">{{ formatCompactLabel(app.app_name || `服务${app.app_id}`, 8, 3) }}</span>
                    </el-tooltip>
...
import {
  Plus, Search, Refresh, Document, VideoPlay, Delete, Setting, Check
...
// 数据加载函数
const formatCompactLabel = (value, prefixLength = 10, suffixLength = 4) => {
...
.table-cell-with-icon .release-title-text {
  display: block;
  min-width: 0;
  max-width: 100%;
  white-space: normal;
...
.service-item {
  ...
  background: linear-gradient(135deg, rgba(37, 99, 235, 0.22), rgba(30, 64, 175, 0.12));
