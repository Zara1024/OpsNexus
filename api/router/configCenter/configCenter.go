...
import (
	...
	taskController "dodevops-api/api/task/controller"
...
func RegisterConfigCenterRoutes(router *gin.RouterGroup) {
	...
	configAnsibleCtrl := taskController.NewConfigAnsibleController(taskService.NewConfigAnsibleService(common.GetDB()))
	...
	router.GET("/config/ansible", configAnsibleCtrl.List)
	router.POST("/config/ansible", configAnsibleCtrl.Create)
	router.GET("/config/ansible/:id", configAnsibleCtrl.Get)
	router.PUT("/config/ansible/:id", configAnsibleCtrl.Update)
	router.DELETE("/config/ansible/:id", configAnsibleCtrl.Delete)
}
