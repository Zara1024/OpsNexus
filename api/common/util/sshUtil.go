...
func (s *SSHUtil) GetSystemInfo(auth *SSHConfig) (map[string]string, error) {
	...
	info["memory"] = normalizeMemoryCapacityFromMeminfoKiB(info["memory"])

	return info, nil
}

func normalizeMemoryCapacityFromMeminfoKiB(value string) string {
	normalized := strings.TrimSpace(value)
	if normalized == "" {
		return ""
	}

	memoryKiB, err := strconv.ParseFloat(normalized, 64)
	if err != nil || memoryKiB <= 0 {
		return value
	}

	memoryGiB := memoryKiB / 1024 / 1024
	return strconv.FormatInt(int64(math.Ceil(memoryGiB)), 10)
}
...
# 获取内存大小(保留 MemTotal 原始 kB，后续由 Go 统一归一)
memory=$(awk '/^MemTotal:/{print $2}' /proc/meminfo 2>/dev/null || echo "unknown")
