...
// GetJobParameters 获取Jenkins任务参数
// @Summary 获取Jenkins任务参数
// @Description 获取指定任务的构建参数定义
// @Tags Jenkins
// @Accept json
// @Produce json
// @Param serverId path int true "服务器ID"
// @Param jobName path string true "任务名称"
// @Security ApiKeyAuth
// @Success 200 {object} result.Result{data=model.JobConfigResponse}
// @Router /jenkins/{serverId}/jobs/{jobName}/parameters [get]
func (jc *JenkinsController) GetJobParameters(c *gin.Context) {
	serverIDStr := c.Param("serverId")
	serverID, err := strconv.ParseUint(serverIDStr, 10, 32)
	if err != nil {
		result.Failed(c, 400, "无效的服务器ID")
	...
	jc.jenkinsService.GetJobParameters(c, uint(serverID), jobName)
}
