...
type applicationNameRow struct {
	ID   uint   `json:"id"`
	Name string `json:"name"`
}

type applicationEnvStatRow struct {
	AppID                      uint `json:"app_id"`
	TotalEnvironmentCount      int  `json:"total_environment_count"`
	ConfiguredEnvironmentCount int  `json:"configured_environment_count"`
}

type applicationDeploymentStatRow struct {
	AppID   uint `json:"app_id"`
...
type applicationLatestDeploymentRow struct {
	AppID         uint      `json:"app_id"`
...
type applicationRecentDeploymentRow struct {
	DeploymentID  uint      `json:"deployment_id"`
	Title         string    `json:"title"`
	CreatorID     uint      `json:"creator_id"`
	CreatorName   string    `json:"creator_name"`
...
func (s *ApplicationService) applyApplicationFilters(query *gorm.DB, req *model.ApplicationListRequest) *gorm.DB {
	if req == nil {
		return query
	}
	...
	if req.Keyword != "" {
		...
		query = query.Where(
			"(app_application.name LIKE ? OR app_application.code LIKE ? OR app_application.description LIKE ? OR app_application.programming_lang LIKE ?)",
			like, like, like, like,
		)
	}
	if req.Name != "" {
		query = query.Where("app_application.name LIKE ?", "%"+strings.TrimSpace(req.Name)+"%")
	}
	if req.Code != "" {
		query = query.Where("app_application.code LIKE ?", "%"+strings.TrimSpace(req.Code)+"%")
	}
	if req.ProgrammingLang != "" {
		query = query.Where("app_application.programming_lang = ?", strings.TrimSpace(req.ProgrammingLang))
	}
	if req.BusinessGroupID != nil {
		query = query.Where("app_application.business_group_id = ?", *req.BusinessGroupID)
	}
	if req.BusinessDeptID != nil {
		query = query.Where("app_application.business_dept_id = ?", *req.BusinessDeptID)
	}
	if req.Status != nil {
...
func (s *ApplicationService) buildApplicationFilterQuery(req *model.ApplicationListRequest) *gorm.DB {
	return s.applyApplicationFilters(s.db.Table("app_application"), req)
}

func (s *ApplicationService) getCurrentOperator(c *gin.Context) (uint, string, error) {
	...
	return admin.ID, name, nil
}

func (s *ApplicationService) getApplicationStatusText(status int) string {
	switch status {
	case 1:
		return "未激活"
	case 2:
...
func (s *ApplicationService) getTaskStatusText(status int) string {
	switch status {
	case 1:
		return "未部署"
	case 2:
...
func (s *ApplicationService) normalizeAppEnvDeployConfig(cfg model.AppEnvDeployConfig) model.AppEnvDeployConfig {
...
func (s *ApplicationService) isAppEnvReleaseGovernanceConfigured(cfg model.AppEnvDeployConfig) bool {
	return cfg.ClusterID > 0 &&
		strings.TrimSpace(cfg.Namespace) != "" &&
		strings.TrimSpace(cfg.WorkloadType) != "" &&
		strings.TrimSpace(cfg.WorkloadName) != ""
}

func (s *ApplicationService) buildAppEnvAIDiagnosisPath(historyID uint, workloadName, namespaceName string) string {
	if historyID == 0 {
		return ""
	}
	...
	values.Set("source", "release-governance")
	values.Set("targetId", strconv.FormatUint(uint64(historyID), 10))
...
func (s *ApplicationService) buildAppEnvK8sWorkloadPath(cfg model.AppEnvDeployConfig, action string) string {
	if !s.isAppEnvReleaseGovernanceConfigured(cfg) {
		return ""
	}

	values := url.Values{}
	values.Set("clusterId", strconv.FormatUint(uint64(cfg.ClusterID), 10))
	values.Set("namespace", cfg.Namespace)
	values.Set("type", cfg.WorkloadType)
	values.Set("name", cfg.WorkloadName)
...
func (s *ApplicationService) loadClusterName(clusterID uint) string {
	if clusterID == 0 {
		return ""
	}
	...
	var row clusterRow
	if err := s.db.Table("k8s_cluster").Select("name").Where("id = ?", clusterID).Take(&row).Error; err != nil {
		return ""
	}
	return strings.TrimSpace(row.Name)
}

func (s *ApplicationService) loadLatestCapacitySuggestionSnapshot(cfg model.AppEnvDeployConfig) *model.AppEnvCapacitySuggestionSummary {
	if !s.isAppEnvReleaseGovernanceConfigured(cfg) {
		return nil
	}

	var entity k8sModel.WorkloadCapacitySuggestionHistoryEntity
	if err := s.db.Where(
	...
	summary := &model.AppEnvCapacitySuggestionSummary{
		HistoryID:       entity.ID,
		RiskLevel:       strings.TrimSpace(entity.RiskLevel),
		GeneratedAt:     entity.CreatedAt.Format("2006-01-02 15:04:05"),
		GeneratedBy:     strings.TrimSpace(entity.GeneratedBy),
		FollowUpWindow:  strings.TrimSpace(entity.FollowUpWindow),
		AlertCenterPath: strings.TrimSpace(entity.AlertCenterPath),
		AIDiagnosisPath: s.buildAppEnvAIDiagnosisPath(entity.ID, entity.WorkloadName, entity.NamespaceName),
		K8sWorkloadPath: s.buildAppEnvK8sWorkloadPath(cfg, "governance"),
	}

	var autoscaling *k8sModel.WorkloadAutoscalingSummary
	if strings.TrimSpace(entity.Autoscaling) != "" && strings.TrimSpace(entity.Autoscaling) != "null" {
		if err := json.Unmarshal([]byte(entity.Autoscaling), &autoscaling); err == nil && autoscaling != nil {
			summary.AutoscalingEnabled = autoscaling.Enabled
			summary.Warnings = append(summary.Warnings, autoscaling.Warnings...)
		}
	}
...
func (s *ApplicationService) buildReleaseGovernanceSummary(cfg model.AppEnvDeployConfig) *model.AppEnvReleaseGovernanceSummary {
	cfg = s.normalizeAppEnvDeployConfig(cfg)
	configured := s.isAppEnvReleaseGovernanceConfigured(cfg)
	if !cfg.ReleaseGovernanceEnabled && !configured {
		return nil
	}

	summary := &model.AppEnvReleaseGovernanceSummary{
		Configured:   configured,
		Enabled:      cfg.ReleaseGovernanceEnabled,
		ClusterID:    cfg.ClusterID,
		ClusterName:  s.loadClusterName(cfg.ClusterID),
	...
	if !configured {
		summary.Warnings = append(summary.Warnings, "已开启发布治理，但未配置 K8s 工作负载映射")
		if summary.Enabled && summary.PreCheckMode == "strict" {
			summary.Blocking = true
			summary.BlockingReason = "未配置 K8s 工作负载映射"
		}
		return summary
	}

	latest := s.loadLatestCapacitySuggestionSnapshot(cfg)
	summary.LatestCapacitySuggestion = latest
	if latest == nil {
		summary.Warnings = append(summary.Warnings, "尚未生成容量建议快照，建议先完成 HPA/容量治理预检")
		if summary.Enabled && summary.PreCheckMode == "strict" {
			summary.Blocking = true
			summary.BlockingReason = "尚未生成容量建议快照"
		}
		return summary
	}

	summary.Warnings = append(summary.Warnings, latest.Warnings...)
	if strings.EqualFold(latest.RiskLevel, "high") {
		summary.Warnings = append(summary.Warnings, "最新容量建议风险等级为 high，建议处理后再发布")
		if summary.Enabled && summary.PreCheckMode == "strict" {
			summary.Blocking = true
			summary.BlockingReason = "最新容量建议风险等级为 high"
		}
	}

	generatedAt, err := time.ParseInLocation("2006-01-02 15:04:05", latest.GeneratedAt, time.Local)
	if err == nil && time.Since(generatedAt) > 24*time.Hour {
		summary.Warnings = append(summary.Warnings, "最新容量建议快照已超过 24h，建议发布前重新生成")
	}

	return summary
}

func (s *ApplicationService) attachReleaseGovernanceToTasks(tasks []model.QuickDeploymentTask) []model.QuickDeploymentTask {
	for i := range tasks {
		tasks[i].ReleaseGovernance = s.buildReleaseGovernanceSummary(tasks[i].JenkinsEnv.DeployConfig)
	}
	return tasks
}
...
func buildJenkinsJobPath(jobName string) string {
	parts := make([]string, 0)
	for _, segment := range strings.Split(strings.Trim(jobName, "/"), "/") {
		segment = strings.TrimSpace(segment)
		if segment == "" {
			continue
		}
		parts = append(parts, "job", url.PathEscape(segment))
	}
	return strings.Join(parts, "/")
}

func (s *ApplicationService) buildJenkinsBaseURL(account *ccmodel.AccountAuth) string {
	if account == nil {
		return ""
	}

	rawHost := strings.TrimSpace(account.Host)
	if rawHost == "" {
		return ""
	}
	...
	port := parsed.Port()
	if port == "" && account.Port > 0 {
		defaultPort := (scheme == "http" && account.Port == 80) || (scheme == "https" && account.Port == 443)
		if !defaultPort {
			port = fmt.Sprintf("%d", account.Port)
		}
	}
...
func (s *ApplicationService) buildJenkinsJobURL(account *ccmodel.AccountAuth, jobName string) string {
	baseURL := s.buildJenkinsBaseURL(account)
	return s.buildJenkinsJobURLByBase(baseURL, jobName)
}

func (s *ApplicationService) buildJenkinsJobURLByBase(baseURL, jobName string) string {
	jobPath := buildJenkinsJobPath(jobName)
	if baseURL == "" || jobPath == "" {
		return ""
	}
	return strings.TrimRight(baseURL, "/") + "/" + jobPath
}

func (s *ApplicationService) buildJenkinsConsoleURL(account *ccmodel.AccountAuth, jobName string, buildNumber int) string {
	jobURL := s.buildJenkinsJobURL(account, jobName)
	if jobURL == "" || buildNumber <= 0 {
		return ""
	}
	return fmt.Sprintf("%s/%d/console", jobURL, buildNumber)
}

func (s *ApplicationService) buildJenkinsProgressiveLogURL(account *ccmodel.AccountAuth, jobName string, buildNumber int, start int) string {
	jobURL := s.buildJenkinsJobURL(account, jobName)
	if jobURL == "" || buildNumber <= 0 {
		return ""
	}
	return fmt.Sprintf("%s/%d/logText/progressiveText?start=%d", jobURL, buildNumber, start)
}

func (s *ApplicationService) loadNameMap(table, nameColumn string, ids []uint) map[uint]string {
	resultMap := make(map[uint]string)
	if len(ids) == 0 {
		return resultMap
	}

	var rows []applicationNameRow
	if err := s.db.Table(table).
		Select(fmt.Sprintf("id, %s as name", nameColumn)).
		Where("id IN ?", ids).
	...
	for _, row := range rows {
		resultMap[row.ID] = row.Name
	}

	return resultMap
}

func (s *ApplicationService) loadApplicationEnvironmentStats(appIDs []uint) map[uint]applicationEnvStatRow {
	statsMap := make(map[uint]applicationEnvStatRow)
	if len(appIDs) == 0 {
		return statsMap
	}

	var rows []applicationEnvStatRow
	if err := s.db.Table("app_jenkins_env").
		Select(`
			app_id,
			COUNT(*) AS total_environment_count,
			SUM(CASE WHEN jenkins_server_id IS NOT NULL AND jenkins_server_id > 0 AND job_name <> '' THEN 1 ELSE 0 END) AS configured_environment_count
		`).
	...
	for _, row := range rows {
		statsMap[row.AppID] = row
	}

	return statsMap
}

func (s *ApplicationService) loadApplicationDeploymentStats(appIDs []uint) map[uint]applicationDeploymentStatRow {
	statsMap := make(map[uint]applicationDeploymentStatRow)
	if len(appIDs) == 0 {
		return statsMap
	}

	var rows []applicationDeploymentStatRow
	if err := s.db.Table("quick_deployment_tasks").
		Select(`
			app_id,
			COUNT(*) AS total,
			SUM(CASE WHEN status = 3 THEN 1 ELSE 0 END) AS success,
			SUM(CASE WHEN status IN (4, 5) THEN 1 ELSE 0 END) AS failed
		`).
		Where("app_id IN ?", appIDs).
		Group("app_id").
		Scan(&rows).Error; err != nil {
...
func (s *ApplicationService) loadLatestDeploymentMap(appIDs []uint) map[uint]*model.ApplicationLatestDeployment {
	latestMap := make(map[uint]*model.ApplicationLatestDeployment)
	if len(appIDs) == 0 {
		return latestMap
	}

	var rows []applicationLatestDeploymentRow
	if err := s.db.Table("quick_deployment_tasks AS t").
		Select(`
			t.app_id,
			...
			t.log_url
		`).
		Joins("JOIN quick_deployments AS d ON d.id = t.deployment_id").
		Where("t.app_id IN ?", appIDs).
		Order("d.created_at DESC, t.id DESC").
		Scan(&rows).Error; err != nil {
		return latestMap
	}

	for _, row := range rows {
		if _, exists := latestMap[row.AppID]; exists {
			continue
		}

		latestMap[row.AppID] = &model.ApplicationLatestDeployment{
			...
			Status:        row.Status,
			StatusText:    s.getTaskStatusText(row.Status),
			BuildNumber:   row.BuildNumber,
			CreatedAt:     row.CreatedAt,
			JenkinsJobURL: row.JenkinsJobURL,
			LogURL:        row.LogURL,
		}
	}
...
func (s *ApplicationService) loadRecentDeploymentsForApp(appID uint, limit int) []model.ApplicationRecentDeployment {
	if appID == 0 || limit <= 0 {
		return nil
	}

	var rows []applicationRecentDeploymentRow
	if err := s.db.Table("quick_deployment_tasks AS t").
		Select(`
			d.id AS deployment_id,
			d.title,
	...
	recent := make([]model.ApplicationRecentDeployment, 0, len(rows))
	for _, row := range rows {
		recent = append(recent, model.ApplicationRecentDeployment{
			DeploymentID:  row.DeploymentID,
			Title:         row.Title,
			CreatorID:     row.CreatorID,
			CreatorName:   row.CreatorName,
...
func (s *ApplicationService) enrichJenkinsEnvs(envs []model.JenkinsEnv) []model.JenkinsEnv {
	if len(envs) == 0 {
		return envs
	}

	serverIDs := make([]uint, 0)
	serverIDMap := make(map[uint]struct{})
	for _, env := range envs {
		if env.JenkinsServerID == nil || *env.JenkinsServerID == 0 {
			continue
		}
		if _, exists := serverIDMap[*env.JenkinsServerID]; exists {
			continue
		}
		serverIDMap[*env.JenkinsServerID] = struct{}{}
		serverIDs = append(serverIDs, *env.JenkinsServerID)
	}

	serverMap := make(map[uint]*ccmodel.AccountAuth)
	if len(serverIDs) > 0 {
	...
	for i := range envs {
		envs[i].DeployConfig = s.normalizeAppEnvDeployConfig(envs[i].DeployConfig)
		envs[i].IsConfigured = envs[i].JenkinsServerID != nil && *envs[i].JenkinsServerID > 0 && strings.TrimSpace(envs[i].JobName) != ""
		envs[i].ReleaseGovernance = s.buildReleaseGovernanceSummary(envs[i].DeployConfig)
		if envs[i].JenkinsServerID == nil || *envs[i].JenkinsServerID == 0 {
			continue
		}

		server := serverMap[*envs[i].JenkinsServerID]
		if server == nil {
			continue
		}

		envs[i].ServerName = server.Alias
		envs[i].JenkinsJobURL = s.buildJenkinsJobURL(server, envs[i].JobName)
	}

	return envs
}

func (s *ApplicationService) enrichApplications(apps []model.Application, includeRecentDeployments bool) []model.Application {
	if len(apps) == 0 {
		return apps
	}
	...
	for _, app := range apps {
		...
		if _, exists := groupIDSet[app.BusinessGroupID]; !exists {
			groupIDSet[app.BusinessGroupID] = struct{}{}
			groupIDs = append(groupIDs, app.BusinessGroupID)
		}
		if _, exists := deptIDSet[app.BusinessDeptID]; !exists {
			deptIDSet[app.BusinessDeptID] = struct{}{}
			deptIDs = append(deptIDs, app.BusinessDeptID)
		}
	}

	groupNameMap := s.loadNameMap("cmdb_group", "name", groupIDs)
	deptNameMap := s.loadNameMap("sys_dept", "dept_name", deptIDs)
	envStatMap := s.loadApplicationEnvironmentStats(appIDs)
	deploymentStatMap := s.loadApplicationDeploymentStats(appIDs)
	latestDeploymentMap := s.loadLatestDeploymentMap(appIDs)

	for i := range apps {
		if name := strings.TrimSpace(groupNameMap[apps[i].BusinessGroupID]); name != "" {
			apps[i].BusinessGroupName = name
		} else {
			apps[i].BusinessGroupName = fmt.Sprintf("未匹配业务组(%d)", apps[i].BusinessGroupID)
		}

		if name := strings.TrimSpace(deptNameMap[apps[i].BusinessDeptID]); name != "" {
			apps[i].BusinessDeptName = name
		} else {
			apps[i].BusinessDeptName = fmt.Sprintf("未匹配部门(%d)", apps[i].BusinessDeptID)
		}

		if envStat, exists := envStatMap[apps[i].ID]; exists {
			apps[i].ConfiguredEnvironmentCount = envStat.ConfiguredEnvironmentCount
			apps[i].TotalEnvironmentCount = envStat.TotalEnvironmentCount
		}

		if deploymentStat, exists := deploymentStatMap[apps[i].ID]; exists {
			apps[i].DeploymentStats = model.ApplicationDeploymentStats{
				Total:   deploymentStat.Total,
				Success: deploymentStat.Success,
				Failed:  deploymentStat.Failed,
			}
		}

		if latestDeployment, exists := latestDeploymentMap[apps[i].ID]; exists {
			apps[i].LatestDeployment = latestDeployment
		}

		if includeRecentDeployments {
			apps[i].RecentDeployments = s.loadRecentDeploymentsForApp(apps[i].ID, 10)
		}

		apps[i].JenkinsEnvs = s.enrichJenkinsEnvs(apps[i].JenkinsEnvs)
	}

	return apps
}

func (s *ApplicationService) buildApplicationOverviewSummary(req *model.ApplicationListRequest, total int64) *model.ApplicationOverviewSummary {
	summary := &model.ApplicationOverviewSummary{
		TotalApplications: int(total),
	}

	activeQuery := s.buildApplicationFilterQuery(req).Where("app_application.status = ?", 2)
	inactiveQuery := s.buildApplicationFilterQuery(req).Where("app_application.status = ?", 1)
	abnormalQuery := s.buildApplicationFilterQuery(req).Where("app_application.status = ?", 3)

	var activeCount int64
	var inactiveCount int64
	var abnormalCount int64
	activeQuery.Count(&activeCount)
	inactiveQuery.Count(&inactiveCount)
	abnormalQuery.Count(&abnormalCount)
	summary.ActiveApplications = int(activeCount)
	summary.InactiveApplications = int(inactiveCount)
	summary.AbnormalApplications = int(abnormalCount)

	var totalEnvCount int64
	var configuredEnvCount int64
	var totalDeploymentCount int64
	var successDeploymentCount int64
	var failedDeploymentCount int64

	s.db.Table("app_jenkins_env").
		Where("app_id IN (?)", s.buildApplicationFilterQuery(req).Select("app_application.id")).
		Count(&totalEnvCount)

	s.db.Table("app_jenkins_env").
		Where("app_id IN (?) AND jenkins_server_id IS NOT NULL AND jenkins_server_id > 0 AND job_name <> ''", s.buildApplicationFilterQuery(req).Select("app_application.id")).
		Count(&configuredEnvCount)

	s.db.Table("quick_deployment_tasks").
		Where("app_id IN (?)", s.buildApplicationFilterQuery(req).Select("app_application.id")).
		Count(&totalDeploymentCount)

	s.db.Table("quick_deployment_tasks").
		Where("app_id IN (?) AND status = 3", s.buildApplicationFilterQuery(req).Select("app_application.id")).
		Count(&successDeploymentCount)

	s.db.Table("quick_deployment_tasks").
		Where("app_id IN (?) AND status IN (4, 5)", s.buildApplicationFilterQuery(req).Select("app_application.id")).
		Count(&failedDeploymentCount)

	summary.TotalEnvironments = int(totalEnvCount)
	summary.ConfiguredEnvironments = int(configuredEnvCount)
	summary.TotalDeployments = int(totalDeploymentCount)
	summary.SuccessfulDeployments = int(successDeploymentCount)
	summary.FailedDeployments = int(failedDeploymentCount)
...
func (s *ApplicationService) GetApplicationList(c *gin.Context, req *model.ApplicationListRequest) {
	...
	apps = s.enrichApplications(apps, false)

	response := &model.ApplicationListResponse{
		Total:   int(total),
		List:    apps,
		Summary: s.buildApplicationOverviewSummary(req, total),
	}
...
func (s *ApplicationService) GetApplicationDetail(c *gin.Context, id uint) {
	...
	enriched := s.enrichApplications([]model.Application{*app}, true)
	if len(enriched) == 0 {
		result.Success(c, app)
		return
	}

	result.Success(c, enriched[0])
}
...
func (s *ApplicationService) CreateJenkinsEnv(c *gin.Context, req *model.CreateJenkinsEnvRequest) {
	...
	env := &model.JenkinsEnv{
		...
		JobName:         req.JobName,
		DeployConfig: func() model.AppEnvDeployConfig {
			if req.DeployConfig == nil {
				return model.AppEnvDeployConfig{}
			}
			return s.normalizeAppEnvDeployConfig(*req.DeployConfig)
		}(),
	}
...
func (s *ApplicationService) UpdateAppJenkinsEnv(c *gin.Context, appID uint, envID uint, req *model.UpdateJenkinsEnvRequest) {
	...
	if req.DeployConfig != nil {
		updates["deploy_config"] = s.normalizeAppEnvDeployConfig(*req.DeployConfig)
	}
...
func (s *ApplicationService) GetAppJenkinsEnvs(c *gin.Context, appID uint) {
	...
	result.Success(c, s.enrichJenkinsEnvs(envs))
}
...
func (s *ApplicationService) GetApplicationsForDeployment(c *gin.Context, req *model.GetApplicationsForDeploymentRequest) {
	...
	for _, app := range apps {
		...
		releaseGovernance := func() *model.AppEnvReleaseGovernanceSummary {
			if targetEnv == nil {
				return nil
			}
			return s.buildReleaseGovernanceSummary(targetEnv.DeployConfig)
		}()
		...
		} else if releaseGovernance != nil && releaseGovernance.Blocking {
			deployApp.CanDeploy = false
			deployApp.Reason = "发布治理拦截: " + releaseGovernance.BlockingReason
		} else {
		...
		deployApp.ReleaseGovernance = releaseGovernance
...
func (s *ApplicationService) CreateQuickDeployment(c *gin.Context, req *model.CreateQuickDeploymentRequest) {
	...
	creatorID, creatorName, err := s.getCurrentOperator(c)
	if err != nil {
		result.Failed(c, 401, "获取当前用户失败: "+err.Error())
		return
	}
	...
	for index, appReq := range req.Applications {
		...
		releaseGovernance := s.buildReleaseGovernanceSummary(targetEnv.DeployConfig)
		if releaseGovernance != nil && releaseGovernance.Blocking {
			tx.Rollback()
			result.Failed(c, 400, fmt.Sprintf("应用 %s 的 %s 环境发布治理拦截: %s", app.Name, appReq.Environment, releaseGovernance.BlockingReason))
			return
		}
		...
		if account, accountErr := accountDao.GetByID(*targetEnv.JenkinsServerID); accountErr == nil {
			jobURL = s.buildJenkinsJobURL(account, targetEnv.JobName)
		}
...
func (s *ApplicationService) GetQuickDeploymentList(c *gin.Context, req *model.QuickDeploymentListRequest) {
	...
	baseQuery := s.db.Model(&model.QuickDeployment{}).
		Joins("LEFT JOIN quick_deployment_tasks ON quick_deployments.id = quick_deployment_tasks.deployment_id")

	// 构建查询条件
	if req.Title != "" {
		baseQuery = baseQuery.Where("quick_deployments.title LIKE ?", "%"+strings.TrimSpace(req.Title)+"%")
	}
	if req.BusinessGroupID != nil {
		baseQuery = baseQuery.Where("quick_deployments.business_group_id = ?", *req.BusinessGroupID)
	}
	if req.BusinessDeptID != nil {
		baseQuery = baseQuery.Where("quick_deployments.business_dept_id = ?", *req.BusinessDeptID)
	}
	if req.Environment != "" {
		baseQuery = baseQuery.Where("quick_deployment_tasks.environment = ?", strings.TrimSpace(req.Environment))
	}
	if req.Status != nil {
		baseQuery = baseQuery.Where("quick_deployments.status = ?", *req.Status)
	}
	if req.CreatorID != nil {
		baseQuery = baseQuery.Where("quick_deployments.creator_id = ?", *req.CreatorID)
	}

	// 获取总数
	if err := baseQuery.Session(&gorm.Session{}).
		Distinct("quick_deployments.id").
	...
	if err := baseQuery.Session(&gorm.Session{}).
		Select("quick_deployments.*").
		Group("quick_deployments.id").
		Preload("Tasks").
		Preload("Tasks.JenkinsEnv").
		Offset(offset).
		Limit(req.PageSize).
		Order("quick_deployments.created_at DESC").
	...
	for i := range response.List {
		response.List[i].Tasks = s.attachReleaseGovernanceToTasks(response.List[i].Tasks)
	}
...
func (s *ApplicationService) GetQuickDeploymentDetail(c *gin.Context, id uint) {
	...
	deployment.Tasks = s.attachReleaseGovernanceToTasks(deployment.Tasks)
...
func (s *ApplicationService) updateTaskFailure(task *model.QuickDeploymentTask, startTime time.Time, errorMessage string) {
	...
	// Immediate startup failures still need to roll up the parent deployment state.
	s.checkAndUpdateDeploymentStatus(task.ID)
}
	...
	for _, envReq := range envs {
		if envReq.ID != nil {
			...
			if existingEnv, exists := existingEnvMap[envID]; exists {
				...
				if envReq.DeployConfig != nil {
					updates["deploy_config"] = s.normalizeAppEnvDeployConfig(*envReq.DeployConfig)
				}
			...
			createReq := &model.CreateJenkinsEnvRequest{
				...
				DeployConfig:    envReq.DeployConfig,
			}
...
func (s *ApplicationService) ValidateJenkinsJob(c *gin.Context, req *model.ValidateJenkinsJobRequest) {
	...
	// 3. 构建Jenkins API URL
	jobURL := s.buildJenkinsJobURL(&jenkinsServer, req.JobName)
	...
	for _, app := range apps {
		...
		if _, exists := businessLineMap[app.BusinessGroupID]; !exists {
			businessLineMap[app.BusinessGroupID] = &model.BusinessLineServiceTree{
				...
				BusinessGroupName: app.BusinessGroupName,
				ServiceCount:      0,
		...
		for _, env := range app.JenkinsEnvs {
			...
			jenkinsEnvs = append(jenkinsEnvs, model.ServiceJenkinsEnv{
				...
				DeployConfig:      env.DeployConfig,
				ReleaseGovernance: env.ReleaseGovernance,
			})
		}
		...
		serviceNode := model.ServiceTreeNode{
			...
			Code:             app.Code,
			Status:           app.Status,
			StatusText:       s.getApplicationStatusText(app.Status),
			ProgrammingLang:  app.ProgrammingLang,
			BusinessDeptID:   app.BusinessDeptID,
			BusinessDeptName: app.BusinessDeptName,
...
func (s *ApplicationService) executeJenkinsJob(serverID uint, jobName string) (bool, int, string, error) {
	...
	jobURL := s.buildJenkinsJobURL(account, jobName)
	if baseURL == "" || jobURL == "" {
		return false, 0, "", fmt.Errorf("Jenkins地址配置无效")
	}
	...
	// 2. 启动构建（带crumb令牌）
	buildURL := fmt.Sprintf("%s/build", jobURL)
	...
	// 获取构建URL
	logURL := s.buildJenkinsConsoleURL(account, jobName, buildNumber)
...
func (s *ApplicationService) getLatestBuildNumber(baseURL, username, password, jobName string) (int, error) {
	jobBaseURL := s.buildJenkinsJobURLByBase(baseURL, jobName)
	if jobBaseURL == "" {
		return 0, fmt.Errorf("Jenkins任务地址无效")
	}

	jobURL := fmt.Sprintf("%s/api/json?tree=lastBuild[number]", jobBaseURL)
...
func (s *ApplicationService) getBuildStatusDetailed(baseURL, username, password, jobName string, buildNumber int) (bool, string, string, error) {
	jobBaseURL := s.buildJenkinsJobURLByBase(baseURL, jobName)
	if jobBaseURL == "" {
		return false, "", "", fmt.Errorf("Jenkins任务地址无效")
	}

	buildURL := fmt.Sprintf("%s/%d/api/json", jobBaseURL, buildNumber)
...
func (s *ApplicationService) GetTaskBuildLog(c *gin.Context, taskID uint, start int) {
	...
	// 获取构建日志
	logURL := s.buildJenkinsProgressiveLogURL(account, task.JenkinsEnv.JobName, task.BuildNumber, start)
	if logURL == "" {
		result.Failed(c, 500, "Jenkins日志地址无效")
		return
	}
...
func (s *ApplicationService) GetBusinessGroupOptions(c *gin.Context) {
	...
	query := `
		SELECT
			a.business_group_id,
			COALESCE(g.name, CONCAT('未匹配业务组(', a.business_group_id, ')')) AS business_group_name,
			COUNT(*) AS service_count
		FROM app_application a
		LEFT JOIN cmdb_group g ON g.id = a.business_group_id
		GROUP BY a.business_group_id, g.name
	...
	for _, group := range businessGroups {
		...
		deptQuery := `
			SELECT
				a.business_dept_id,
				COALESCE(d.dept_name, CONCAT('未匹配部门(', a.business_dept_id, ')')) AS business_dept_name,
				COUNT(*) AS service_count
			FROM app_application a
			LEFT JOIN sys_dept d ON d.id = a.business_dept_id
			WHERE a.business_group_id = ?
			GROUP BY a.business_dept_id, d.dept_name
			HAVING service_count > 0
			ORDER BY a.business_dept_id
		`
...
func (s *ApplicationService) GetAppEnvironment(c *gin.Context, req *model.GetAppEnvironmentRequest) {
	...
	if targetEnv != nil {
		...
		response.DeployConfig = s.normalizeAppEnvDeployConfig(targetEnv.DeployConfig)
		response.ReleaseGovernance = s.buildReleaseGovernanceSummary(targetEnv.DeployConfig)
		...
		if targetEnv.JenkinsServerID != nil && *targetEnv.JenkinsServerID > 0 {
			...
			if account, err := accountDao.GetByID(*targetEnv.JenkinsServerID); err == nil {
				...
				response.JenkinsJobURL = s.buildJenkinsJobURL(account, targetEnv.JobName)
			}
		}
	} else {
		// 环境未配置
