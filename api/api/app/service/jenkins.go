...
type IJenkinsService interface {
	...
	GetJobParameters(c *gin.Context, serverID uint, jobName string)
...
type jenkinsJobParametersPayload struct {
	Name       string                      `json:"name"`
	Properties []jenkinsJobPropertyPayload `json:"property"`
}

type jenkinsJobPropertyPayload struct {
	Class                string                             `json:"_class"`
	ParameterDefinitions []jenkinsParameterDefinitionPayload `json:"parameterDefinitions"`
}

type jenkinsParameterDefinitionPayload struct {
	Name                  string                          `json:"name"`
	Type                  string                          `json:"type"`
	Description           string                          `json:"description"`
	DefaultParameterValue *jenkinsDefaultParameterPayload `json:"defaultParameterValue"`
	Choices               interface{}                     `json:"choices"`
}

type jenkinsDefaultParameterPayload struct {
...
func normalizeJenkinsChoices(raw interface{}) []string {
	switch value := raw.(type) {
	case []string:
		return append([]string(nil), value...)
	case []interface{}:
		choices := make([]string, 0, len(value))
...
func decodeJobParameters(body io.Reader) (string, []model.BuildParameter, error) {
	var payload jenkinsJobParametersPayload
	if err := json.NewDecoder(body).Decode(&payload); err != nil {
		return "", nil, err
	}

	parameters := make([]model.BuildParameter, 0)
	for _, property := range payload.Properties {
		for _, definition := range property.ParameterDefinitions {
			parameter := model.BuildParameter{
				Name:        definition.Name,
				Type:        definition.Type,
				Description: definition.Description,
				Choices:     normalizeJenkinsChoices(definition.Choices),
			}
	...
	return payload.Name, parameters, nil
}
...
// GetJobParameters 获取任务参数定义
func (s *JenkinsServiceImpl) GetJobParameters(c *gin.Context, serverID uint, jobName string) {
	client, err := s.getJenkinsClient(serverID)
	if err != nil {
		result.Failed(c, 500, "获取Jenkins客户端失败: "+err.Error())
		return
	...
	if jobPath == "" {
		result.Failed(c, 400, "无效的任务名称")
		return
	}

	endpoint := fmt.Sprintf("/%s/api/json?tree=name,property[_class,parameterDefinitions[name,type,description,defaultParameterValue[value],choices]]", jobPath)
	resp, err := client.makeRequest("GET", endpoint, nil)
	if err != nil {
		result.Failed(c, 500, "获取任务参数失败: "+err.Error())
		return
	}
	defer resp.Body.Close()
	...
	decodedJobName, parameters, err := decodeJobParameters(resp.Body)
	if err != nil {
		result.Failed(c, 500, "解析Jenkins任务参数失败: "+err.Error())
		return
	}

	serverName := ""
	if account, accountErr := s.accountDao.GetByID(serverID); accountErr == nil {
		serverName = account.Alias
	}
	if strings.TrimSpace(decodedJobName) == "" {
		decodedJobName = jobName
	}

	result.Success(c, model.JobConfigResponse{
