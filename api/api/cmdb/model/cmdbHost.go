...
type CmdbHost struct {
	...
	DeviceType   string     `gorm:"column:device_type;varchar(20);default:'linux';comment:'设备类型(switch/router/windows/linux)'" json:"deviceType"`
	RemoteDomain string     `gorm:"column:remote_domain;varchar(100);comment:'域名(仅Windows RDP需要)'" json:"remoteDomain"`
	RDPPort      int        `gorm:"column:rdp_port;default:3389;comment:'RDP端口'" json:"rdpPort"`
	RDPUsername  string     `gorm:"column:rdp_username;varchar(100);comment:'RDP用户名'" json:"rdpUsername"`
	RDPPassword  string     `gorm:"column:rdp_password;varchar(255);comment:'RDP密码'" json:"rdpPassword"`
	RDPDomain    string     `gorm:"column:rdp_domain;varchar(100);comment:'RDP域'" json:"rdpDomain"`
...
type CreateCmdbHostDto struct {
	...
	SSHName      string `json:"sshName"`                      // SSH登录用户名
	SSHIP        string `validate:"required" json:"sshIp"`    // 管理IP(公网或私网IP)
	SSHPort      int    `json:"sshPort"`                      // SSH端口(默认22)
	SSHKeyID     uint   `json:"sshKeyId"`                     // SSH凭据ID(从ecsAuth表获取)
	Remark       string `json:"remark"`                       // 备注信息(可选)
	DeviceType   string `json:"deviceType"`                   // 设备类型
	RemoteDomain string `json:"remoteDomain"`                 // Windows RDP地址/域名
	RDPPort      int    `json:"rdpPort"`                      // RDP端口
	RDPUsername  string `json:"rdpUsername"`                  // RDP用户名
	RDPPassword  string `json:"rdpPassword"`                  // RDP密码
	RDPDomain    string `json:"rdpDomain"`                    // RDP域
}
...
// BatchHostConnectivityDto represents a batch connectivity test request.
type BatchHostConnectivityDto struct {
	HostIDs []uint `validate:"required" json:"hostIds"`
}

// HostConnectivityCheckItem is one batch connectivity result row.
type HostConnectivityCheckItem struct {
...
// BatchHostConnectivityResult returns the batch connectivity summary.
type BatchHostConnectivityResult struct {
	Total       int                         `json:"total"`
	Reachable   int                         `json:"reachable"`
	Unreachable int                         `json:"unreachable"`
	Items       []HostConnectivityCheckItem `json:"items"`
}
