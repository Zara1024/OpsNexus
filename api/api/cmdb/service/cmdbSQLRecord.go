...
// BatchDeleteCmdbSqlLogByIds 根据ID列表批量删除日志
func (s *CmdbSQLRecordService) BatchDeleteCmdbSqlLogByIds(c *gin.Context, dto model.BatchDeleteCmdbSqlLogDto) {
	if err := s.recordDao.BatchDeleteByIds(dto.Ids); err != nil {
		result.Failed(c, 500, err.Error())
		return
	}
	result.Success(c, nil)
