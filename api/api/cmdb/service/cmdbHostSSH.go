...
type CmdbHostSSHServiceInterface interface {
	...
	PreviewCommandRisk(c *gin.Context, hostID uint, command string) (*systemService.TerminalCommandRiskAssessment, error)
...
func (s *CmdbHostSSHServiceImpl) ConnectTerminal(c *gin.Context, hostID uint) (*websocket.WebSSH, error) {
	...
	if admin, adminErr := jwt.GetAdmin(c); adminErr == nil && admin != nil {
		recorder, recorderErr := systemService.NewTerminalAuditRecorder(systemService.TerminalAuditRecorderOptions{
			AdminID:   admin.ID,
			Username:  admin.Username,
			HostID:    host.ID,
			HostName:  firstNonEmptySSH(host.HostName, host.Name),
			HostIP:    firstNonEmptySSH(host.SSHIP, host.PrivateIP, host.PublicIP),
			SSHUser:   host.SSHName,
			ClientIP:  c.ClientIP(),
		...
		if recorderErr != nil {
			return nil, fmt.Errorf("鍒涘缓SSH瀹¤浼氳瘽澶辫触: %v", recorderErr)
		}
		webSSH.SetAuditRecorder(recorder)
	}
...
func (s *CmdbHostSSHServiceImpl) PreviewCommandRisk(_ *gin.Context, hostID uint, command string) (*systemService.TerminalCommandRiskAssessment, error) {
	if _, err := dao.NewCmdbHostSSHDao().GetHostSSHInfo(hostID); err != nil {
		return nil, fmt.Errorf("获取主机信息失败: %v", err)
	}
	assessment := systemService.ClassifyTerminalCommandRisk(command)
...
func sanitizeCommandOutput(output []byte) string {
	...
	for _, line := range lines {
		...
		if strings.Contains(trimmed, "{{end}}") || strings.Contains(trimmed, ".Destination}}") {
			continue
		}
...
func normalizeNonInteractiveShellCommand(command string) string {
	...
	for alias, expanded := range aliasMap {
		if trimmed == alias {
			return expanded
		}
		if strings.HasPrefix(trimmed, alias+" ") {
			return expanded + trimmed[len(alias):]
		}
	}
...
func (s *CmdbHostSSHServiceImpl) ExecuteCommand(c *gin.Context, hostID uint, command string) (*CommandResponse, error) {
	...
	// 执行命令并获取输出
	normalizedCommand := normalizeNonInteractiveShellCommand(command)
	output, err := session.CombinedOutput(normalizedCommand)
	if err != nil {
		return nil, fmt.Errorf("命令执行失败: %v", err)
	}

	// 清理输出
	finalOutput := sanitizeCommandOutput(output)
...
func firstNonEmptySSH(values ...string) string {
