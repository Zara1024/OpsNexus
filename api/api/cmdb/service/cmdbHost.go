...
type CmdbHostServiceInterface interface {
	...
	BatchTestHostConnectivity(c *gin.Context, dto *model.BatchHostConnectivityDto)                            // 批量测试主机连通性
}
...
func (s *CmdbHostServiceImpl) GetCmdbHostListWithPage(c *gin.Context, page, pageSize int) {
	...
	for _, host := range list {
		...
		vos = append(vos, buildCmdbHostVo(host, group.Name, false))
	}
...
func (s *CmdbHostServiceImpl) CreateCmdbHost(c *gin.Context, dto *model.CreateCmdbHostDto) {
	...
	deviceType := normalizeHostDeviceType(dto.DeviceType)
	if dto.SSHPort <= 0 {
		dto.SSHPort = 22
	}
	if dto.RDPPort <= 0 {
	...
	host := model.CmdbHost{
		...
		Name:         dto.HostName,
		GroupID:      dto.GroupID,
		SSHIP:        strings.TrimSpace(dto.SSHIP),
		SSHName:      strings.TrimSpace(dto.SSHName),
		...
		RemoteDomain: strings.TrimSpace(dto.RemoteDomain),
		RDPPort:      dto.RDPPort,
		RDPUsername:  strings.TrimSpace(dto.RDPUsername),
		RDPPassword:  strings.TrimSpace(dto.RDPPassword),
		RDPDomain:    strings.TrimSpace(dto.RDPDomain),
	}
	...
	if deviceType == "windows" {
		result.Success(c, gin.H{
			"id":     host.ID,
			"status": host.Status,
			"msg":    "Windows主机创建成功，已保存RDP配置；如需主机同步或命令执行，请后续补充SSH能力或依赖Agent。",
		})
		return
	}
	...
	if host.SSHIP == "" || host.SSHName == "" || host.SSHKeyID == 0 {
		result.Success(c, gin.H{
			"id":     host.ID,
			"status": host.Status,
			"msg":    "主机创建成功，当前未配置完整SSH信息，已跳过自动采集。",
		})
		return
	}
	...
	auth, err := authDao.GetEcsAuthById(dto.SSHKeyID)
	if err != nil {
		result.Success(c, gin.H{
			"id":     host.ID,
			"status": host.Status,
			"msg":    "主机创建成功，但SSH凭据不存在，已跳过自动采集。",
		})
		return
	}
...
func (s *CmdbHostServiceImpl) GetCmdbHostById(c *gin.Context, id uint) {
	...
	group, _ := s.groupDao.GetCmdbGroupById(host.GroupID)
	vo := buildCmdbHostVo(host, group.Name, true)
	result.Success(c, vo)
}
...
func (s *CmdbHostServiceImpl) GetCmdbHostByName(c *gin.Context, name string) {
	...
	group, _ := s.groupDao.GetCmdbGroupById(host.GroupID)
	vo := buildCmdbHostVo(host, group.Name, false)
	result.Success(c, vo)
}
...
func (s *CmdbHostServiceImpl) SyncHostInfo(c *gin.Context, id uint) {
	...
	if normalizeHostDeviceType(host.DeviceType) == "windows" {
		result.FailedWithCode(c, constant.INVALID_PARAMS, "Windows主机暂不支持通过当前Linux SSH脚本自动同步，请通过Agent或手工维护资产信息")
		return
	}
...
func buildCmdbHostVo(host model.CmdbHost, groupName string, includeSecrets bool) model.CmdbHostVo {
	vo := model.CmdbHostVo{
		ID:           host.ID,
		...
		DeviceType:   normalizeHostDeviceType(host.DeviceType),
		RemoteDomain: host.RemoteDomain,
		RDPPort:      normalizeRDPPort(host.RDPPort),
		RDPUsername:  host.RDPUsername,
		RDPDomain:    host.RDPDomain,
		SupportsSSH:  strings.TrimSpace(host.SSHIP) != "" && strings.TrimSpace(host.SSHName) != "" && host.SSHKeyID > 0,
		CreateTime:   host.CreateTime,
		ExpireTime:   host.ExpireTime,
		UpdateTime:   host.UpdateTime,
	}
	if includeSecrets {
		vo.RDPPassword = host.RDPPassword
	}
	return vo
}

func normalizeHostDeviceType(deviceType string) string {
