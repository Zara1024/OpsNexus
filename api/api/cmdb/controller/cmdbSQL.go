...
// @Accept json
// @Produce json
// @Param data body model.CreateCmdbSQLDto true "数据库信息"
// @Success 200 {object} result.Result
// @Router /api/v1/cmdb/database [post]
// @Security ApiKeyAuth
func (c *CmdbSQLController) CreateDatabase(ctx *gin.Context) {
	var req model.CreateCmdbSQLDto
	if err := ctx.ShouldBindJSON(&req); err != nil {
		result.Failed(ctx, 400, "参数错误")
		return
	}

	db, err := c.service.CreateDatabase(req)
	if err != nil {
		if service.IsInvalidCmdbSQLParams(err) {
			result.Failed(ctx, 400, "创建失败: "+err.Error())
			return
		}
...
// @Accept json
// @Produce json
// @Param data body model.UpdateCmdbSQLDto true "数据库信息(必须包含ID)"
// @Success 200 {object} result.Result
// @Router /api/v1/cmdb/database [put]
// @Security ApiKeyAuth
func (c *CmdbSQLController) UpdateDatabase(ctx *gin.Context) {
	var req model.UpdateCmdbSQLDto
	if err := ctx.ShouldBindJSON(&req); err != nil {
		result.Failed(ctx, 400, "参数错误")
		return
	}
	...
	db, err := c.service.UpdateDatabase(req)
	if err != nil {
		if service.IsInvalidCmdbSQLParams(err) {
			result.Failed(ctx, 400, "更新失败: "+err.Error())
			return
		}
		if errors.Is(err, gorm.ErrRecordNotFound) {
			result.Failed(ctx, 404, "database not found")
			return
		}
