...
const hostTemplateName = "host_template.xlsx"
...
func (c *CmdbHostController) DownloadHostTemplate(ctx *gin.Context) {
	filePath, err := resolveHostTemplatePath()
	if err != nil {
	...
	ctx.Header("Content-Disposition", "attachment; filename="+hostTemplateName)
	ctx.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	ctx.File(filePath)
}

func resolveHostTemplatePath() (string, error) {
	workingDir, _ := os.Getwd()
	execPath, _ := os.Executable()
	execDir := filepath.Dir(execPath)

	candidates := []string{
		filepath.Join(workingDir, "upload", "xlsl", "host.xlsx"),
		filepath.Join(workingDir, "api", "upload", "xlsl", "host.xlsx"),
		filepath.Join(execDir, "upload", "xlsl", "host.xlsx"),
		filepath.Join(execDir, "api", "upload", "xlsl", "host.xlsx"),
	}
...
// GetCmdbHostListWithPage 获取主机列表(分页)
// @Summary 获取主机列表(分页)
...
// ImportHostsFromExcel 从Excel导入主机
// @Summary 从Excel导入主机
...
func (c *CmdbHostController) ImportHostsFromExcel(ctx *gin.Context) {
	...
	workbook, err := excelize.OpenFile(tempFilePath)
	if err != nil {
	...
	for i, row := range rows {
		...
		hosts = append(hosts, model.ExcelHostTemplate{
...
// GetCmdbHostsByGroupId 根据分组ID获取主机列表
...
// GetCmdbHostsByHostNameLike 根据主机名称模糊查询
// @Summary 根据主机名称模糊查询
...
func (c *CmdbHostController) BatchTestHostConnectivity(ctx *gin.Context) {
	var dto model.BatchHostConnectivityDto
	if err := ctx.ShouldBindJSON(&dto); err != nil {
		result.Failed(ctx, constant.INVALID_PARAMS, "参数错误")
		return
	}
	c.service.BatchTestHostConnectivity(ctx, &dto)
}
