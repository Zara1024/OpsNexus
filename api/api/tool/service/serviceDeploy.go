...
const (
	templateRootRelative        = "common/templates"
	projectTemplateRootRelative = "api/common/templates"
	servicesTemplateFile        = "services.json"
)

func loadServicesConfig() (*toolModel.ServicesConfig, error) {
	data, _, err := readTemplateFile(servicesTemplateFile)
	if err != nil {
...
func readTemplateFile(relativePath string) ([]byte, string, error) {
	templatePath, err := resolveTemplatePath(relativePath)
	if err != nil {
		return nil, "", err
	}
...
func resolveTemplatePath(relativePath string) (string, error) {
	cleanRelativePath, err := cleanTemplateRelativePath(relativePath)
	if err != nil {
		return "", err
	}

	baseDirs := templateBaseDirs()
	checkedPaths := make([]string, 0, len(baseDirs))
	for _, baseDir := range baseDirs {
		candidate := filepath.Join(baseDir, cleanRelativePath)
	...
	return "", fmt.Errorf("模板文件不存在: %s (checked: %s)", cleanRelativePath, strings.Join(checkedPaths, ", "))
}

func cleanTemplateRelativePath(relativePath string) (string, error) {
	cleanRelativePath := filepath.Clean(relativePath)
	if cleanRelativePath == "." || cleanRelativePath == "" {
		return "", fmt.Errorf("模板路径不能为空")
	}
	if filepath.IsAbs(cleanRelativePath) {
		return "", fmt.Errorf("模板路径必须为相对路径: %s", relativePath)
	}
	if cleanRelativePath == ".." || strings.HasPrefix(cleanRelativePath, ".."+string(os.PathSeparator)) {
		return "", fmt.Errorf("模板路径不允许越级访问: %s", relativePath)
	}
	return cleanRelativePath, nil
}

func templateBaseDirs() []string {
	baseDirs := make([]string, 0, 8)
	seen := make(map[string]struct{})

	addBaseDir := func(path string) {
		if path == "" {
			return
		}
		...
		baseDirs = append(baseDirs, cleanPath)
	}

	if workingDir, err := os.Getwd(); err == nil {
		addBaseDir(filepath.Join(workingDir, templateRootRelative))
		addBaseDir(filepath.Join(workingDir, projectTemplateRootRelative))
	}

	if executablePath, err := os.Executable(); err == nil {
		executableDir := filepath.Dir(executablePath)
		addBaseDir(filepath.Join(executableDir, templateRootRelative))
		addBaseDir(filepath.Join(executableDir, projectTemplateRootRelative))
		addBaseDir(filepath.Join(executableDir, "..", templateRootRelative))
		addBaseDir(filepath.Join(executableDir, "..", projectTemplateRootRelative))
	}

	addBaseDir(templateRootRelative)
	addBaseDir(projectTemplateRootRelative)
...
func (s ServiceDeployServiceImpl) executeBinaryDeploy(
	...
	// 8. 读取并执行安装脚本
	scriptData, templatePath, err := readTemplateFile(versionInfo.File)
	if err != nil {
...
func (s ServiceDeployServiceImpl) executeContainerDeploy(
	...
	templateData, templatePath, err := readTemplateFile(versionInfo.File)
	if err != nil {
