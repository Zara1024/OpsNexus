...
type IKubeClusterService interface {
	...
	GetClusterList(c *gin.Context, page, size int, name, version string, status *int)
...
func (s *KubeClusterServiceImpl) GetCluster(c *gin.Context, id uint) {
	...
	// 获取集群节点信息（通过K8s API）
	nodes, nodeErr := s.loadClusterNodes(cluster)
	if nodeErr != nil {
...
func (s *KubeClusterServiceImpl) GetClusterList(c *gin.Context, page, size int, name, version string, status *int) {
	clusters, total, err := s.dao.List(page, size, name, version, status)
	if err != nil {
...
func (s *KubeClusterServiceImpl) GetClusterStatus(c *gin.Context, id uint) {
	...
	// 获取节点信息
	nodes, _ := s.loadClusterNodes(cluster)
	summary := s.calculateClusterSummary(nodes)
...
func (s *KubeClusterServiceImpl) loadClusterNodes(cluster *model.KubeCluster) ([]model.NodeInfo, error) {
	if strings.TrimSpace(cluster.Credential) == "" {
		return nil, fmt.Errorf("闆嗙兢鍑瘉涓嶈兘涓虹┖")
	}

	config, err := clientcmd.RESTConfigFromKubeConfig([]byte(cluster.Credential))
	if err != nil {
		return nil, fmt.Errorf("瑙ｆ瀽 kubeconfig 澶辫触: %v", err)
	}

	clientset, err := kubernetes.NewForConfig(config)
	if err != nil {
		return nil, fmt.Errorf("鍒涘缓 Kubernetes client 澶辫触: %v", err)
	}

	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()

	return s.getDetailedNodes(ctx, clientset)
}
...
func (s *KubeClusterServiceImpl) createSelfBuiltCluster(c *gin.Context, req *model.CreateKubeClusterRequest) {
	...
	if req.ClusterType == 1 { // 1=自建集群
		go func() {
			...
			// ??????????????
			if req.AutoDeploy {
				if err := s.startK8sTaskAsync(taskID); err != nil {
					fmt.Printf("\u81ea\u52a8\u542f\u52a8 K8s \u4efb\u52a1\u5931\u8d25: %v\n", err)
					s.dao.UpdateStatus(cluster.ID, model.ClusterStatusStopped)
					return
				}

				fmt.Printf("AutoDeploy=%v, taskID=%d started\n", req.AutoDeploy, taskID)
				return
			}

			fmt.Printf("AutoDeploy=%v, task created and waiting for manual start\n", req.AutoDeploy)
		}()
	}
...
func (s *KubeClusterServiceImpl) getClusterNodes(cluster *model.KubeCluster) ([]model.NodeInfo, error) {
	return s.loadClusterNodes(cluster)
}
...
func (s *KubeClusterServiceImpl) startK8sTaskAsync(taskID uint) error {
	if err := s.taskService.StartJobByID(taskID); err != nil {
		return fmt.Errorf("\u542f\u52a8 K8s \u4efb\u52a1\u5931\u8d25: %w", err)
	}

	fmt.Printf("K8s task %d started\n", taskID)
	return nil
}
