...
func resolveServiceTargetPort(targetPort string, defaultPort int32) intstr.IntOrString {
	trimmed := strings.TrimSpace(targetPort)
	if trimmed == "" {
		return intstr.FromInt(int(defaultPort))
	}

	return intstr.Parse(trimmed)
}
...
func (s *K8sServiceServiceImpl) CreateService(c *gin.Context, clusterId uint, namespaceName string, req *model.CreateServiceRequest) {
	...
	for _, portSpec := range req.Ports {
		...
		port.TargetPort = resolveServiceTargetPort(portSpec.TargetPort, portSpec.Port)
