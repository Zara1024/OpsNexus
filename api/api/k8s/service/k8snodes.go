...
type IK8sNodesService interface {
	GetNodes(c *gin.Context, clusterId uint, name, status, role string)
...
func (s *K8sNodesServiceImpl) GetNodes(c *gin.Context, clusterId uint, name, status, role string) {
	// 获取集群信息
	...
	nodes = s.filterNodes(nodes, name, status, role)

	result.Success(c, nodes)
}

func (s *K8sNodesServiceImpl) filterNodes(nodes []model.K8sNode, name, status, role string) []model.K8sNode {
	...
	filtered := make([]model.K8sNode, 0, len(nodes))
	for _, node := range nodes {
		if name != "" && !strings.Contains(strings.ToLower(node.Name), name) {
			continue
		}
		if status != "" && !matchesNodeStatus(node, status) {
			continue
		}
		if role != "" && !matchesNodeRole(node, role) {
			continue
		}
...
func matchesNodeStatus(node model.K8sNode, status string) bool {
	currentStatus := strings.ToLower(strings.TrimSpace(node.Status))

	if status == "schedulingdisabled" {
		return node.Scheduling.Unschedulable
	}

	return currentStatus == status
}

func matchesNodeRole(node model.K8sNode, role string) bool {
	if strings.ToLower(strings.TrimSpace(node.Configuration.Role)) == role {
		return true
	}

	for _, item := range strings.Split(strings.ToLower(node.Roles), ",") {
