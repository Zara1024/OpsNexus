...
type IK8sIngressService interface {
	...
	TestIngressBackend(c *gin.Context, clusterId uint, namespaceName string, req *model.IngressBackendTestRequest)
}
...
// TestIngressBackend 测试 Ingress 后端服务
func (s *K8sIngressServiceImpl) TestIngressBackend(c *gin.Context, clusterId uint, namespaceName string, req *model.IngressBackendTestRequest) {
	clientset, err := s.getK8sClient(clusterId)
	if err != nil {
		result.Failed(c, http.StatusInternalServerError, "连接K8s集群失败: "+err.Error())
		return
	...
	svc, err := clientset.CoreV1().Services(namespaceName).Get(context.TODO(), req.ServiceName, metav1.GetOptions{})
	if err != nil {
		result.Failed(c, http.StatusNotFound, "Service不存在: "+err.Error())
		return
	}

	servicePort, found := resolveIngressBackendServicePort(svc, req.ServicePort)
	details := model.IngressBackendTestDetails{
		ServiceName: req.ServiceName,
		ServicePort: servicePort,
		TestPath:    normalizeIngressBackendTestPath(req.Path),
		TestHost:    strings.TrimSpace(req.Host),
		Method:      normalizeIngressBackendTestMethod(req.Method),
	}

	eps, err := clientset.CoreV1().Endpoints(namespaceName).Get(context.TODO(), req.ServiceName, metav1.GetOptions{})
	if err == nil {
		populateIngressBackendEndpointDetails(eps, &details)
	}

	if !found {
		result.Success(c, model.IngressBackendTestResponse{
			Success:    false,
			Status:     "\u5931\u8d25",
			Message:    "\u672a\u627e\u5230\u5339\u914d\u7684 Service \u7aef\u53e3",
			Suggestion: "\u8bf7\u68c0\u67e5 Ingress \u540e\u7aef servicePort \u662f\u5426\u4e0e Service \u66b4\u9732\u7aef\u53e3\u4e00\u81f4",
			Details:    details,
		})
		return
	}

	if details.EndpointsReady == 0 {
		result.Success(c, buildIngressBackendTestResult(0, 0, details, nil))
		return
	}
	...
	if clusterIP == "" || strings.EqualFold(clusterIP, "None") {
		result.Success(c, model.IngressBackendTestResponse{
			Success:    false,
			Status:     "\u5931\u8d25",
			Message:    "Service ClusterIP \u4e0d\u53ef\u7528",
			Suggestion: "\u8bf7\u68c0\u67e5\u8be5 Service \u662f\u5426\u4e3a Headless Service\uff0c\u6216\u4f7f\u7528 ClusterIP \u7c7b\u578b\u670d\u52a1",
			Details:    details,
		})
		return
	}
	...
	requestURL := fmt.Sprintf("http://%s:%d%s", clusterIP, servicePort, details.TestPath)
	httpReq, err := http.NewRequest(details.Method, requestURL, nil)
	if err != nil {
		result.Success(c, buildIngressBackendTestResult(0, 0, details, err))
		return
	}
	if details.TestHost != "" {
		httpReq.Host = details.TestHost
		httpReq.Header.Set("Host", details.TestHost)
	}

	startTime := time.Now()
	httpResp, err := (&http.Client{Timeout: time.Duration(timeoutSeconds) * time.Second}).Do(httpReq)
	responseTime := time.Since(startTime).Milliseconds()
	if err != nil {
		result.Success(c, buildIngressBackendTestResult(0, responseTime, details, err))
		return
	}
	defer httpResp.Body.Close()

	bodyBytes, readErr := io.ReadAll(io.LimitReader(httpResp.Body, 64*1024))
	if readErr != nil {
		result.Success(c, buildIngressBackendTestResult(httpResp.StatusCode, responseTime, details, readErr))
		return
	}

	details.ResponseHeaders = cloneIngressBackendResponseHeaders(httpResp.Header)
	details.ResponseBody = string(bodyBytes)

	result.Success(c, buildIngressBackendTestResult(httpResp.StatusCode, responseTime, details, nil))
}
...
func normalizeIngressBackendTestMethod(method string) string {
...
func normalizeIngressBackendTestPath(path string) string {
...
func resolveIngressBackendServicePort(svc *corev1.Service, requestedPort int32) (int32, bool) {
	if svc == nil {
	...
	return svc.Spec.Ports[0].Port, requestedPort <= 0
}

func populateIngressBackendEndpointDetails(eps *corev1.Endpoints, details *model.IngressBackendTestDetails) {
	if eps == nil || details == nil {
		return
	}

	for _, subset := range eps.Subsets {
		details.EndpointsReady += len(subset.Addresses)
		details.EndpointsTotal += len(subset.Addresses) + len(subset.NotReadyAddresses)

		for _, addr := range subset.Addresses {
			if addr.TargetRef != nil && addr.TargetRef.Name != "" {
				details.PodsTested = append(details.PodsTested, addr.TargetRef.Name)
				continue
			}
			if addr.IP != "" {
				details.PodsTested = append(details.PodsTested, addr.IP)
			}
		}
	}
}

func cloneIngressBackendResponseHeaders(headers http.Header) map[string][]string {
	if len(headers) == 0 {
		return nil
	}
...
func buildIngressBackendTestResult(statusCode int, responseTime int64, details model.IngressBackendTestDetails, requestErr error) model.IngressBackendTestResponse {
	response := model.IngressBackendTestResponse{
		Success:      false,
		Status:       "\u5931\u8d25",
	...
	switch {
	case details.EndpointsReady == 0:
		response.Message = "\u540e\u7aef Endpoint \u672a\u5c31\u7eea"
		response.Suggestion = "\u8bf7\u68c0\u67e5 Service Selector\u3001Pod \u5c31\u7eea\u72b6\u6001\u548c\u5bb9\u5668\u63a2\u9488\u914d\u7f6e"
	case requestErr != nil:
		response.Message = "\u8bbf\u95ee\u540e\u7aef\u670d\u52a1\u5931\u8d25: " + requestErr.Error()
		response.Suggestion = "\u8bf7\u68c0\u67e5 Service ClusterIP \u53ef\u8fbe\u6027\u3001\u76d1\u542c\u7aef\u53e3\u548c NetworkPolicy \u914d\u7f6e"
	case statusCode >= http.StatusOK && statusCode < http.StatusMultipleChoices:
		response.Success = true
		response.Status = "\u6210\u529f"
		response.Message = "\u540e\u7aef\u670d\u52a1\u54cd\u5e94\u6b63\u5e38"
	default:
		response.Message = fmt.Sprintf("\u540e\u7aef\u670d\u52a1\u8fd4\u56de HTTP %d", statusCode)
		response.Suggestion = "\u8bf7\u68c0\u67e5 Ingress \u8def\u7531\u3001Service \u7aef\u53e3\u548c\u5e94\u7528\u5185\u90e8\u5904\u7406\u903b\u8f91"
	}
