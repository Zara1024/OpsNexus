...
type IK8sNamespaceService interface {
	GetNamespaces(c *gin.Context, clusterId uint, name, status string)
...
func (s *K8sNamespaceServiceImpl) GetNamespaces(c *gin.Context, clusterId uint, name, status string) {
	...
	if s.cacheService != nil {
		...
		if err == nil {
			filteredNamespaces := s.filterNamespaces(cachedResponse.Namespaces, name, status)
			result.Success(c, model.NamespaceListResponse{
	...
	filteredNamespaces := s.filterNamespaces(namespaces, name, status)
	result.Success(c, model.NamespaceListResponse{
		Namespaces: filteredNamespaces,
		Total:      len(filteredNamespaces),
	})
...
func (s *K8sNamespaceServiceImpl) filterNamespaces(namespaces []model.K8sNamespace, name, status string) []model.K8sNamespace {
	...
	filtered := make([]model.K8sNamespace, 0, len(namespaces))
	for _, namespace := range namespaces {
		if name != "" && !strings.Contains(strings.ToLower(namespace.Name), name) {
			continue
		}
		if status != "" && strings.ToLower(strings.TrimSpace(namespace.Status)) != status {
			continue
		}
