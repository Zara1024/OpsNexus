...
type K8sWorkload struct {
	...
	Autoscaling   *WorkloadAutoscalingSummary `json:"autoscaling,omitempty"` // HPA摘要
	Status        string                      `json:"status"`                // 状态
	CreatedAt     string                      `json:"createdAt"`             // 创建时间
...
// WorkloadAutoscalingSummary 工作负载自动扩缩容摘要
type WorkloadAutoscalingSummary struct {
	Enabled         bool                              `json:"enabled"`                   // 是否启用HPA
	Name            string                            `json:"name,omitempty"`            // HPA名称
	MinReplicas     int32                             `json:"minReplicas,omitempty"`     // 最小副本
	MaxReplicas     int32                             `json:"maxReplicas,omitempty"`     // 最大副本
	CurrentReplicas int32                             `json:"currentReplicas,omitempty"` // 当前副本
	DesiredReplicas int32                             `json:"desiredReplicas,omitempty"` // 期望副本
	LastScaleTime   string                            `json:"lastScaleTime,omitempty"`   // 最近扩缩容时间
	Metrics         []WorkloadAutoscalingMetricStatus `json:"metrics,omitempty"`         // 指标摘要
	Conditions      []WorkloadAutoscalingCondition    `json:"conditions,omitempty"`      // HPA条件
	Behavior        *WorkloadAutoscalingBehavior      `json:"behavior,omitempty"`        // 扩缩容行为
	Warnings        []string                          `json:"warnings,omitempty"`        // 治理提示
}

// WorkloadAutoscalingMetricSpec 自动扩缩容指标配置
type WorkloadAutoscalingMetricSpec struct {
	ResourceName string `json:"resourceName" binding:"required"` // cpu/memory
	TargetType   string `json:"targetType" binding:"required"`   // Utilization/AverageValue
	TargetValue  string `json:"targetValue" binding:"required"`  // 阈值
}

// WorkloadAutoscalingMetricStatus 自动扩缩容指标状态
type WorkloadAutoscalingMetricStatus struct {
	ResourceName              string `json:"resourceName"`                        // cpu/memory
	TargetType                string `json:"targetType"`                          // Utilization/AverageValue
	TargetValue               string `json:"targetValue"`                         // 目标阈值
	CurrentAverageValue       string `json:"currentAverageValue,omitempty"`       // 当前平均值
	CurrentAverageUtilization *int32 `json:"currentAverageUtilization,omitempty"` // 当前平均利用率
}

// WorkloadAutoscalingCondition 自动扩缩容条件
type WorkloadAutoscalingCondition struct {
	Type               string `json:"type"`               // 条件类型
	Status             string `json:"status"`             // 状态
	LastTransitionTime string `json:"lastTransitionTime"` // 最近变更时间
	Reason             string `json:"reason"`             // 原因
	Message            string `json:"message"`            // 信息
}

// WorkloadAutoscalingBehavior 自动扩缩容行为配置
type WorkloadAutoscalingBehavior struct {
	ScaleUp   *WorkloadAutoscalingBehaviorRule `json:"scaleUp,omitempty"`   // 扩容策略
	ScaleDown *WorkloadAutoscalingBehaviorRule `json:"scaleDown,omitempty"` // 缩容策略
}

// WorkloadAutoscalingBehaviorRule HPA行为规则
type WorkloadAutoscalingBehaviorRule struct {
	StabilizationWindowSeconds *int32                             `json:"stabilizationWindowSeconds,omitempty"` // 稳定窗口
	SelectPolicy               string                             `json:"selectPolicy,omitempty"`               // 选择策略
	Policies                   []WorkloadAutoscalingScalingPolicy `json:"policies,omitempty"`                   // 限速策略
}

// WorkloadAutoscalingScalingPolicy HPA限速策略
type WorkloadAutoscalingScalingPolicy struct {
	Type          string `json:"type"`          // Pods/Percent
	Value         int32  `json:"value"`         // 数值
	PeriodSeconds int32  `json:"periodSeconds"` // 周期秒数
}
...
// WorkloadAutoscalingResponse 工作负载自动扩缩容详情
type WorkloadAutoscalingResponse struct {
	Supported    bool                        `json:"supported"`             // 当前工作负载是否支持HPA
	Exists       bool                        `json:"exists"`                // 是否已存在HPA
	WorkloadType string                      `json:"workloadType"`          // 工作负载类型
	WorkloadName string                      `json:"workloadName"`          // 工作负载名称
	Namespace    string                      `json:"namespace"`             // 命名空间
	Autoscaling  *WorkloadAutoscalingSummary `json:"autoscaling,omitempty"` // 自动扩缩容详情
}

// WorkloadCapacityAlertEvent 容量建议关联告警事件
type WorkloadCapacityAlertEvent struct {
...
// WorkloadCapacityIncident 容量建议关联事件工单
type WorkloadCapacityIncident struct {
...
// WorkloadCapacityAlertSummary 工作负载关联告警摘要
type WorkloadCapacityAlertSummary struct {
	...
	RecentEvents       []WorkloadCapacityAlertEvent `json:"recentEvents,omitempty"`
	RecentIncidents    []WorkloadCapacityIncident   `json:"recentIncidents,omitempty"`
}

// WorkloadCapacityRecommendation 容量建议动作
type WorkloadCapacityRecommendation struct {
...
// WorkloadCapacityPolicy 建议策略
type WorkloadCapacityPolicy struct {
	Type              string `json:"type"`
...
// WorkloadCapacitySuggestionResponse 工作负载容量建议
type WorkloadCapacitySuggestionResponse struct {
	...
	GeneratedByID      uint                             `json:"generatedById,omitempty"`
	...
	AlertKeyword       string                           `json:"alertKeyword"`
	AlertCenterPath    string                           `json:"alertCenterPath"`
	AlertCenterQuery   WorkloadCapacityAlertCenterQuery `json:"alertCenterQuery"`
	Autoscaling        *WorkloadAutoscalingSummary      `json:"autoscaling,omitempty"`
	AlertSummary       WorkloadCapacityAlertSummary     `json:"alertSummary"`
	RiskLevel          string                           `json:"riskLevel"`
	RecommendedActions []WorkloadCapacityRecommendation `json:"recommendedActions"`
	RecommendedPolicy  WorkloadCapacityPolicy           `json:"recommendedPolicy"`
	WatchMetrics       []string                         `json:"watchMetrics"`
	FollowUpWindow     string                           `json:"followUpWindow"`
}

// WorkloadGovernanceRuleHit describes one governance rule evaluation result.
type WorkloadGovernanceRuleHit struct {
	Type    string `json:"type"`
...
// WorkloadGovernanceStage describes one workload governance stage.
type WorkloadGovernanceStage struct {
	Key               string   `json:"key"`
	Title             string   `json:"title"`
	Status            string   `json:"status"`
	Summary           string   `json:"summary"`
	Blocking          bool     `json:"blocking"`
	PrimaryActionText string   `json:"primaryActionText,omitempty"`
	PrimaryActionPath string   `json:"primaryActionPath,omitempty"`
	Items             []string `json:"items,omitempty"`
}

// WorkloadGovernanceExtension describes one reserved extension slot.
type WorkloadGovernanceExtension struct {
	Key            string   `json:"key"`
	Title          string   `json:"title"`
	Status         string   `json:"status"`
	Summary        string   `json:"summary"`
...
// WorkloadGovernanceAppLink describes one related application governance target.
type WorkloadGovernanceAppLink struct {
	AppID          uint   `json:"appId"`
	AppName        string `json:"appName"`
	AppCode        string `json:"appCode,omitempty"`
	Environment    string `json:"environment"`
	PreCheckMode   string `json:"preCheckMode,omitempty"`
	ReleaseEnabled bool   `json:"releaseEnabled"`
	DetailPath     string `json:"detailPath,omitempty"`
}

// WorkloadGovernanceOverviewResponse is the unified workload governance workbench payload.
type WorkloadGovernanceOverviewResponse struct {
	ClusterID           uint                                `json:"clusterId"`
	ClusterName         string                              `json:"clusterName"`
	Namespace           string                              `json:"namespace"`
	WorkloadType        string                              `json:"workloadType"`
	...
	BlockingRules       []WorkloadGovernanceRuleHit         `json:"blockingRules,omitempty"`
	WarningRules        []WorkloadGovernanceRuleHit         `json:"warningRules,omitempty"`
	Stages              []WorkloadGovernanceStage           `json:"stages,omitempty"`
	CurrentSuggestion   WorkloadCapacitySuggestionResponse  `json:"currentSuggestion"`
	LatestSnapshot      *WorkloadCapacitySuggestionResponse `json:"latestSnapshot,omitempty"`
	CurrentAutoscaling  *WorkloadAutoscalingSummary         `json:"currentAutoscaling,omitempty"`
	AlertSummary        WorkloadCapacityAlertSummary        `json:"alertSummary"`
	WatchMetrics        []string                            `json:"watchMetrics,omitempty"`
	Extensions          []WorkloadGovernanceExtension       `json:"extensions,omitempty"`
	RelatedApplications []WorkloadGovernanceAppLink         `json:"relatedApplications,omitempty"`
	AlertCenterPath     string                              `json:"alertCenterPath,omitempty"`
	AIDiagnosisPath     string                              `json:"aiDiagnosisPath,omitempty"`
	K8sWorkloadPath     string                              `json:"k8sWorkloadPath,omitempty"`
}
...
// UpsertWorkloadAutoscalingRequest 创建或更新HPA
type UpsertWorkloadAutoscalingRequest struct {
	MinReplicas int32                           `json:"minReplicas" binding:"required"` // 最小副本
	MaxReplicas int32                           `json:"maxReplicas" binding:"required"` // 最大副本
	Metrics     []WorkloadAutoscalingMetricSpec `json:"metrics" binding:"required"`     // 资源指标
	Behavior    *WorkloadAutoscalingBehavior    `json:"behavior,omitempty"`             // 扩缩容行为
}
...
// IngressBackendTestRequest 测试 Ingress 后端服务请求
type IngressBackendTestRequest struct {
	ServiceName string `json:"serviceName" binding:"required"` // 服务名称
	ServicePort int32  `json:"servicePort" binding:"required"` // 服务端口
	Path        string `json:"path"`                           // 测试路径
	Host        string `json:"host"`                           // Host 请求头
	Method      string `json:"method"`                         // 请求方法
	Timeout     int    `json:"timeout"`                        // 超时时间（秒）
}

// IngressBackendTestDetails Ingress 后端测试详情
type IngressBackendTestDetails struct {
	ServiceName     string              `json:"serviceName"`               // 服务名称
	ServicePort     int32               `json:"servicePort"`               // 服务端口
	TestPath        string              `json:"testPath"`                  // 测试路径
	TestHost        string              `json:"testHost"`                  // Host 请求头
	Method          string              `json:"method"`                    // 请求方法
	EndpointsReady  int                 `json:"endpointsReady"`            // 就绪端点数
	EndpointsTotal  int                 `json:"endpointsTotal"`            // 总端点数
	PodsTested      []string            `json:"podsTested,omitempty"`      // 已测试 Pod
	ResponseHeaders map[string][]string `json:"responseHeaders,omitempty"` // 响应头
	ResponseBody    string              `json:"responseBody,omitempty"`    // 响应体
}

// IngressBackendTestResponse Ingress 后端测试响应
type IngressBackendTestResponse struct {
	Success      bool                      `json:"success"`              // 是否成功
	Status       string                    `json:"status"`               // 测试状态
	Message      string                    `json:"message"`              // 结果消息
	Suggestion   string                    `json:"suggestion,omitempty"` // 处理建议
	StatusCode   int                       `json:"statusCode"`           // HTTP 状态码
	ResponseTime int64                     `json:"responseTime"`         // 响应时间（毫秒）
	Details      IngressBackendTestDetails `json:"details"`              // 测试详情
}
