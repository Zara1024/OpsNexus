...
func (ctrl *K8sIngressController) TestIngressBackend(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
	...
	var req model.IngressBackendTestRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		result.Failed(c, 400, "鍙傛暟楠岃瘉澶辫触: "+err.Error())
		return
	}

	ctrl.service.TestIngressBackend(c, uint(clusterId), namespaceName, &req)
}
