...
func (ctrl *K8sWorkloadController) GetWorkloads(c *gin.Context) {
	...
	ctrl.service.GetWorkloads(c, uint(clusterId), namespaceName, workloadType, name)
}
...
func (ctrl *K8sWorkloadController) GetWorkloadAutoscaling(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "无效的集群ID")
	...
	ctrl.service.GetWorkloadAutoscaling(c, uint(clusterId), namespaceName, workloadType, workloadName)
}

// UpsertWorkloadAutoscaling 创建或更新工作负载HPA
func (ctrl *K8sWorkloadController) UpsertWorkloadAutoscaling(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "无效的集群ID")
	...
	var req model.UpsertWorkloadAutoscalingRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		result.Failed(c, 400, "参数验证失败: "+err.Error())
		return
	}

	ctrl.service.UpsertWorkloadAutoscaling(c, uint(clusterId), namespaceName, workloadType, workloadName, &req)
}

// DeleteWorkloadAutoscaling 删除工作负载HPA
func (ctrl *K8sWorkloadController) DeleteWorkloadAutoscaling(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "无效的集群ID")
	...
	ctrl.service.DeleteWorkloadAutoscaling(c, uint(clusterId), namespaceName, workloadType, workloadName)
}

// GetWorkloadCapacitySuggestion 获取工作负载容量建议
func (ctrl *K8sWorkloadController) GetWorkloadCapacitySuggestion(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "无效的集群ID")
	...
	ctrl.service.GetWorkloadCapacitySuggestion(c, uint(clusterId), namespaceName, workloadType, workloadName)
}

// GetWorkloadCapacitySuggestionHistory 获取工作负载容量建议历史
func (ctrl *K8sWorkloadController) GetWorkloadCapacitySuggestionHistory(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "无效的集群ID")
	...
	query := model.WorkloadCapacitySuggestionHistoryQuery{
		PageSize: parseOptionalInt(c.Query("pageSize"), 10),
		PageNum:  parseOptionalInt(c.Query("pageNum"), 1),
	}
	ctrl.service.GetWorkloadCapacitySuggestionHistory(c, uint(clusterId), namespaceName, workloadType, workloadName, query)
}

// GetWorkloadGovernanceOverview 获取工作负载治理工作台概览
func (ctrl *K8sWorkloadController) GetWorkloadGovernanceOverview(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "无效的集群ID")
	...
	ctrl.service.GetWorkloadGovernanceOverview(c, uint(clusterId), namespaceName, workloadType, workloadName)
}
...
func (ctrl *K8sWorkloadController) CreateStatefulSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
		return
	}

	namespaceName := c.Param("namespaceName")
	if namespaceName == "" {
		result.Failed(c, 400, "鍛藉悕绌洪棿鍚嶇О涓嶈兘涓虹┖")
		return
	}

	var req model.CreateStatefulSetRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		result.Failed(c, 400, "鍙傛暟楠岃瘉澶辫触: "+err.Error())
		return
	}

	ctrl.service.CreateStatefulSet(c, uint(clusterId), namespaceName, &req)
}

// DeleteStatefulSet 删除StatefulSet
func (ctrl *K8sWorkloadController) UpdateStatefulSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "閺冪姵鏅ラ惃鍕肠缂囶棷D")
		return
	}

	namespaceName := c.Param("namespaceName")
	if namespaceName == "" {
		result.Failed(c, 400, "閸涜棄鎮曠粚娲？閸氬秶袨娑撳秷鍏樻稉铏光敄")
		return
	}

	statefulSetName := c.Param("statefulSetName")
	if statefulSetName == "" {
		result.Failed(c, 400, "StatefulSet閸氬秶袨娑撳秷鍏樻稉铏光敄")
		return
	}

	var req model.UpdateWorkloadRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		result.Failed(c, 400, "閸欏倹鏆熸宀冪槈婢惰精瑙? "+err.Error())
		return
	}

	ctrl.service.UpdateStatefulSet(c, uint(clusterId), namespaceName, statefulSetName, &req)
}

func (ctrl *K8sWorkloadController) DeleteStatefulSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	statefulSetName := c.Param("statefulSetName")
	if statefulSetName == "" {
		result.Failed(c, 400, "StatefulSet鍚嶇О涓嶈兘涓虹┖")
		return
	}

	ctrl.service.DeleteStatefulSet(c, uint(clusterId), namespaceName, statefulSetName)
}

// ScaleStatefulSet 伸缩StatefulSet
func (ctrl *K8sWorkloadController) ScaleStatefulSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	ctrl.service.ScaleStatefulSet(c, uint(clusterId), namespaceName, statefulSetName, &req)
}

// RestartStatefulSet 重启StatefulSet
func (ctrl *K8sWorkloadController) RestartStatefulSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	ctrl.service.RestartStatefulSet(c, uint(clusterId), namespaceName, statefulSetName)
}

// CreateDaemonSet 创建DaemonSet
func (ctrl *K8sWorkloadController) CreateDaemonSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	var req model.CreateDaemonSetRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		result.Failed(c, 400, "鍙傛暟楠岃瘉澶辫触: "+err.Error())
		return
	}

	ctrl.service.CreateDaemonSet(c, uint(clusterId), namespaceName, &req)
}

// DeleteDaemonSet 删除DaemonSet
func (ctrl *K8sWorkloadController) UpdateDaemonSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "閺冪姵鏅ラ惃鍕肠缂囶棷D")
	...
	if daemonSetName == "" {
		result.Failed(c, 400, "DaemonSet閸氬秶袨娑撳秷鍏樻稉铏光敄")
		return
	}
	...
	ctrl.service.UpdateDaemonSet(c, uint(clusterId), namespaceName, daemonSetName, &req)
}

func (ctrl *K8sWorkloadController) DeleteDaemonSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	daemonSetName := c.Param("daemonSetName")
	if daemonSetName == "" {
		result.Failed(c, 400, "DaemonSet鍚嶇О涓嶈兘涓虹┖")
		return
	}

	ctrl.service.DeleteDaemonSet(c, uint(clusterId), namespaceName, daemonSetName)
}

// RestartDaemonSet 重启DaemonSet
func (ctrl *K8sWorkloadController) RestartDaemonSet(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	ctrl.service.RestartDaemonSet(c, uint(clusterId), namespaceName, daemonSetName)
}

// CreateJob 创建Job
func (ctrl *K8sWorkloadController) CreateJob(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	ctrl.service.CreateJob(c, uint(clusterId), namespaceName, &req)
}

// DeleteJob 删除Job
func (ctrl *K8sWorkloadController) UpdateJob(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "閺冪姵鏅ラ惃鍕肠缂囶棷D")
	...
	if jobName == "" {
		result.Failed(c, 400, "Job閸氬秶袨娑撳秷鍏樻稉铏光敄")
		return
	}
	...
	ctrl.service.UpdateJob(c, uint(clusterId), namespaceName, jobName, &req)
}

func (ctrl *K8sWorkloadController) DeleteJob(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	jobName := c.Param("jobName")
	if jobName == "" {
		result.Failed(c, 400, "Job鍚嶇О涓嶈兘涓虹┖")
		return
	}

	ctrl.service.DeleteJob(c, uint(clusterId), namespaceName, jobName)
}

// CreateCronJob 创建CronJob
func (ctrl *K8sWorkloadController) CreateCronJob(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	var req model.CreateCronJobRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		result.Failed(c, 400, "鍙傛暟楠岃瘉澶辫触: "+err.Error())
		return
	}

	ctrl.service.CreateCronJob(c, uint(clusterId), namespaceName, &req)
}

// DeleteCronJob 删除CronJob
func (ctrl *K8sWorkloadController) UpdateCronJob(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "閺冪姵鏅ラ惃鍕肠缂囶棷D")
	...
	if cronJobName == "" {
		result.Failed(c, 400, "CronJob閸氬秶袨娑撳秷鍏樻稉铏光敄")
		return
	}
	...
	ctrl.service.UpdateCronJob(c, uint(clusterId), namespaceName, cronJobName, &req)
}

func (ctrl *K8sWorkloadController) DeleteCronJob(c *gin.Context) {
	clusterIdStr := c.Param("id")
	clusterId, err := strconv.Atoi(clusterIdStr)
	if err != nil || clusterId <= 0 {
		result.Failed(c, 400, "鏃犳晥鐨勯泦缇D")
	...
	cronJobName := c.Param("cronJobName")
	if cronJobName == "" {
		result.Failed(c, 400, "CronJob鍚嶇О涓嶈兘涓虹┖")
		return
	}

	ctrl.service.DeleteCronJob(c, uint(clusterId), namespaceName, cronJobName)
}
