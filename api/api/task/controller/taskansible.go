...
func (c *TaskAnsibleController) CreateTask(ctx *gin.Context) {
	...
	req := &service.CreateTaskRequest{
		...
		Variables:        variables,
		IsRecurring:      parseBoolInt(ctx.PostForm("is_recurring")),
...
func (c *TaskAnsibleController) GetTaskHistory(ctx *gin.Context) {
	id, err := strconv.Atoi(ctx.Param("id"))
	if err != nil {
		result.Failed(ctx, http.StatusBadRequest, "无效的任务ID")
		return
	...
	c.service.GetTaskHistory(ctx, uint(id), status)
}
...
func (c *TaskAnsibleController) GetTaskHistoryDetail(ctx *gin.Context) {
	...
	if err != nil {
		result.Failed(ctx, http.StatusBadRequest, "无效的历史记录ID")
		return
	}

	c.service.GetTaskHistoryDetail(ctx, uint(id), uint(historyID))
}
