...
func (s *AgentServiceImpl) deployAgentAsync(hostID uint, host model.CmdbHost) error {
	...
	if _, err := exec.LookPath("go"); err != nil {
		log.Printf("当前环境未找到 go，尝试使用预编译 agent binary")
		if fallbackErr := s.copyPrebuiltAgentBinary(buildTarget, binaryPath); fallbackErr != nil {
			return fmt.Errorf("当前环境缺少 go 且未找到预编译 agent binary: %v", fallbackErr)
		}
		log.Printf("已使用预编译 agent binary: %s", binaryPath)
		usedPrebuilt = true
...
func (s *AgentServiceImpl) copyPrebuiltAgentBinary(buildTarget BuildTarget, targetPath string) error {
	prebuiltName := fmt.Sprintf("dodevops-agent-%s-%s%s", buildTarget.GOOS, buildTarget.GOARCH, buildTarget.Suffix)
	candidates := []string{
		filepath.Join(config.Config.ImageSettings.UploadDir, "agents", prebuiltName),
		filepath.Join("upload", "agents", prebuiltName),
		filepath.Join("upload", "agents", fmt.Sprintf("dodevops-agent%s", buildTarget.Suffix)),
	}
	...
	return fmt.Errorf("未找到候选文件: %s", strings.Join(candidates, ", "))
}
