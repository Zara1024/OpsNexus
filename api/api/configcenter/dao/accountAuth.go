...
func (d *AccountAuthDao) ListWithPage(page, pageSize int, alias string, accountType *int) ([]model.AccountAuth, int64, error) {
	var accounts []model.AccountAuth
	var total int64

	offset := (page - 1) * pageSize

	query := common.GetDB().Model(&model.AccountAuth{})
	if alias != "" {
	...
	err := query.Offset(offset).Limit(pageSize).Order("id ASC").Find(&accounts).Error
