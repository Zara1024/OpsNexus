...
func (d *EcsAuthDao) GetEcsAuthListWithPage(page, pageSize int, name string) ([]model.EcsAuth, int64) {
	var list []model.EcsAuth
	var total int64

	offset := (page - 1) * pageSize
	query := d.db.Model(&model.EcsAuth{})
	if name != "" {
		query = query.Where("name LIKE ?", "%"+name+"%")
	}

	query.Count(&total)
	query.Offset(offset).Limit(pageSize).Order("id ASC").Find(&list)
