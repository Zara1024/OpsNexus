<template>
  <el-card>
    ...
    <el-table :data="list" v-loading="loading">
      ...
      <el-table-column label="ClusterIP" min-width="150">
        <template #default="scope">{{ scope.row.spec?.clusterIP || '-' }}</template>
      </el-table-column>
      <el-table-column label="端口" min-width="200">
        <template #default="scope">
          {{ (scope.row.spec?.ports || []).map((p:any) => `${p.port}/${p.protocol}`).join(', ') }}
        </template>
      </el-table-column>
    </el-table>
  </el-card>
...
import { listK8sClustersApi, listK8sNamespacesApi, listServicesApi, type K8sClusterItem } from '@/api/k8s'
