    ...
    <template #header>
      <div class="toolbar">
        ...
        <div class="actions">
          ...
          <el-button v-permission="'k8s:config:secret:save'" type="primary" @click="openCreate">新建</el-button>
        </div>
      </div>
    </template>
    ...
    <el-table :data="list" v-loading="loading">
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="scope">
          <el-button v-permission="'k8s:config:secret:save'" link type="primary" @click="openEdit(scope.row)">编辑</el-button>
          <el-button v-permission="'k8s:config:secret:delete'" link type="danger" @click="remove(scope.row.name)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="dialogVisible" title="保存 Secret" width="760px">
      <el-form :model="form" label-width="90px">
        <el-form-item label="名称"><el-input v-model="form.name" /></el-form-item>
        <el-form-item label="类型"><el-input v-model="form.type" placeholder="Opaque" /></el-form-item>
...
import { deleteSecretApi, listK8sClustersApi, listK8sNamespacesApi, listSecretsApi, saveSecretApi, type K8sClusterItem } from '@/api/k8s'
